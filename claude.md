...
- **민감도 3종 × 2줄**: `민감도·내 주식 (혼합)  누적 +0.09 · 5일 +0.19 · 당일 +0.59` /
  `민감도·내 계좌 (혼합)  누적 +0.04 · 5일 +0.11 · 당일 +0.29`. 전부 원점회귀 베타
  `ΣΔ·Δ벤치 / ΣΔ벤치²` (Δ = 스냅샷 구간 변화). **누적** = 전체 구간, **5일** = 마지막
  `beta_window`(기본 5)구간, **당일** = 마지막 1구간(단일 비율). 표본 3구간 미만(당일은
  1구간)이면 그 값만 None.
  - **내 주식** = Δ주식수익(Rs, 예수금 제외) 기준 — 아래 민감도 그래프가 그리는 값.
    이 줄의 **누적=검정 / 5일=녹색 / 당일=빨강** 색이 곧 그래프 선 색 = 범례 겸용.
  - **내 계좌** = Δ계좌수익(예수금 포함) 기준 — 예수금 쿠션까지 포함한 계좌 전체가 지수에
    얼마나 반응하나. 보통 내 주식보다 작음(예수금이 완충). 사용자 요청(2026-09-02).
  ...
  - "-0.20"은 "벤치 -1%일 때 오히려 +0.2% (역행/방어)", 1.0이면 시장과 똑같이, 0이면 무관.
    **벤치 = 혼합 지수**(`kospi_weight` 없으면 코스피 단독, `sensitivity_basis`가 `"코스피"`).
