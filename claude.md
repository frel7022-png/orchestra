...
- **목적/동작 방식 (2026-08-18 확정, v2 — 최초 버전의 "롤오버 근사" 방식 폐기하고 교체)**:
  156개 관심종목 중 **±3% 이상 움직인 종목만 걸러서 보여주는 스크리너**, 기준(누적/전일)과
  방향(DOWN/UP)을 라디오로 고를 수 있음(Up/Down 기능과 같은 조작 방식). 종목마다
  `watchlist_prices.csv`(`refresh_watchlist_prices()`)에 이렇게 추적한다:
  - **최초가**: 이 종목을 처음 관측한 시점의 "전일 종가" — **영구 보존, 다시는 안 바뀜.**
    네이버 실시간 시세 API가 매 응답마다 주는 `change_pct`(등락률, 정식 전일 종가 대비 %)를
    거꾸로 풀어서 구한다: `최초가 = 최근가 / (1 + change_pct/100)`. (v1에서는 "마지막으로
    새로고침 누른 시각의 가격"을 전일가로 근사했었는데, 네이버가 이미 정확한 전일 종가 대비
    등락률을 계산해서 주고 있다는 걸 뒤늦게 알고 그걸 직접 쓰는 쪽으로 교체함 — 장중 아무 때
    눌러도 정확하고, "장마감 이후에 눌러야 정확하다"는 제약이 없어짐.)
  - **최근가/전일대비**: 매 새로고침마다 그대로 갱신(전일대비는 API가 주는 `change_pct` 그대로,
    직접 계산 안 함).
  - **누적 등락률** = `(최근가-최초가)/최초가`(화면 표시 시점에 계산, 저장 안 함), **전일 등락률**
    = 저장된 `전일대비` 그대로. 오늘 이 기능을 처음 쓰는 종목은 최초가=전일종가라서 두 값이
    같게 나옴 — 의도된 동작.
  ...
  - 나중에 관심종목이 추가되면(watchlist.csv 갱신) 그 종목은 **처음 관측되는 시점이 그 종목만의
    최초가**가 됨(전체가 같은 날 시작할 필요 없음).
  - `watchlist_prices.csv`는 git에 커밋되는 영구 데이터(캐시 파일들과 같은 성격) — **절대 손으로
    편집하거나 삭제하지 말 것**, 최초가가 날아가면 복구 불가능.
  - (참고: "오늘의 순위"라는 TOP20 부가 기능을 한 번 만들었다가, 이 v2 방식으로 바꾸면서
    사용자가 "차라리 없애고 기준/방향 골라보는 쪽으로 하자"고 해서 제거함 — 재도입 안 함.)
