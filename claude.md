...
- Up/Down: 완전 청산한 종목 중 마지막 매도가 대비 변한 것 추적 — **DOWN ±2% / UP ±3%**
...
- Fishing: 관심종목 ±3% 스크리너, 각 줄에 순위 + 전일 대비 순위 변동(▲▼ 또는 신규 진입 NEW)
  ...
  그 종목이 **Up/Down 결과에도 걸려 있으면**(청산 후 DOWN 2%/UP 3% 이탈) Fishing에서 그
...
- **No Refill 재정의 (2026-09-16)**: 원래 정의(예수금−누적실현손익, "매도할 때마다 항상 본전에
  ...
  **"실현손익 대신, 실제로 팔아서 현금화했던 그 돈이 벤치(삼성·하이닉스 제외 혼합지수)를
  ...
  - **핵심 원칙 — "예수금 대 예수금" 비교여야 한다(사용자가 두 차례 시행착오 끝에 확정,
    2026-09-16)**: 벤치 반사실은 **청산(전량매도 완료)된 사이클에만, 그 사이클이 실제로
    투입했던 금액(buy_amt)과 실제 보유기간(최초매수일~청산일)에만** 적용한다.
  ...
  - `portfolio_core.virtual_realized_cum_by_close_date(tx, bench_cum)` 신설 — `_all_cycles`(§6-20)로
    청산된 사이클을 뽑아, 각 사이클마다 `buy_amt × ((1+벤치(청산일))/(1+벤치(최초매수일)) − 1)`을
    계산해 청산일 기준으로 누적한 날짜→누적가상실현손익 dict를 반환. 사이클 내부 부분매도
    타이밍까지는 안 쪼개고 청산일에 그 사이클의 가상손익 전체를 한 번에 반영(§6-20
  ...
  - `bench_cum`이 없거나 청산된 사이클이 없으면 가상실현손익이 전부 0 → 자연스럽게 예전 정의
    (실현손익 0% 가정)로 축소. `bench_cum=None`으로 부르면(기존 호출부·회귀테스트 그대로)
    완전히 예전 동작과 동일.
  - **2026-09-16 실측**: 청산된 사이클들의 벤치 기준 가상실현손익 누적 −104,095원(실제
...
- **UI**(`ui_portfolio_tab.py`): plotly x-unified hover, **선 4개, 차트만(밑 설명줄 없음)**.
  ...
  - **No Refill** (**파랑 `DOWN_COLOR`, 실선**, Refill보다 더 가파르게 하락) = 위 새 정의(청산
...
- 회귀 테스트 `test_seed_engine_series_tracks_cash_and_cost`(bench_cum 없을 때 예전 동작 그대로),
  `test_virtual_realized_cum_by_close_date_scales_to_actual_buy_amount`(청산 사이클의 buy_amt·
...
- **동기**: "물타기가 성공했다/실패했다"가 아니라 **"애초에 물 안 타고 한 번에 끝났으면
  좋았을 판단이었나"**를 재는 지표. 사용자 정의: 한 번 사서 한 번에 전량청산(FA, §6-20)한
...
- **위치/형식**: 포트폴리오 탭 일일거래 요약(daily-trade-box, 요약카드 sc-bot 안) 맨 밑
  ...
  1. `Total {전량청산 완료 수}/{전체 사이클 수, open 포함} ({비율}%)`
  2. `FA {수}({승률}%) · MA {수}({비율}%) · MO {나눠팔아서 전량청산한 수}({비율}%)`
  — FA+MA+MO(닫힌 것만) 합이 Total의 분자와 정확히 일치(2026-09-16 사용자가 "79+11=90인데
  왜 120이냐"고 지적해서 MO_closed를 추가로 노출 — 30이 나눠팔기로 청산된 몫이었음). 괄호
...
- **함수**: `portfolio_core.compute_fa_win_rate(tx)` — `_all_cycles`/`_cycle_bucket`(§6-20)을
  ...
  용도로는 날짜가 필요 없었음). 반환: win(FA)·total·n_out(전량청산 완료)·ma·mo_closed·
...
- **Top Traded를 Price Brackets 하위 expander로, 전종목 표시 + 하루 1회 고정 새로고침
  ...
  **청산 완료된 사이클이 있는 종목 전부**를 보여줌(사용자: "121개가 5회도 있고 1회도
  ...
  - **버그(같은 날 실제로 겪음) — 날짜 게이트가 "오늘 이미 새로고침했는지"만 보고 "그 결과가
    최신 스키마인지"는 안 봐서, 같은 날 안에 캐시 구조 자체를 바꾸면(2026-09-16 당일
    Selection Index → 평균 강도 비교 → 정리성 거래 필터를 연달아 추가) 새로고침을 다시
    눌러도 날짜가 같다는 이유로 그냥 반환돼 새 필드가 하루 종일 안 보임**(사용자가
    "그 평균 오른거 대비 이익본 퍼센트는 안 나오는데?"라고 지적해서 발견). **고침**:
    `_TOP_TRADED_CACHE_VERSION` 상수를 캐시에 `cache_version`으로 같이 저장하고, 게이트
    조건에 `cache_version == _TOP_TRADED_CACHE_VERSION`도 추가 — 날짜가 같아도 버전이
    다르면 강제로 다시 계산한다. **캐시 딕셔너리에 새 필드를 추가하는 등 스키마를 바꿀
    때마다 이 숫자를 올릴 것** — 안 올리면 이 버그가 그대로 재발한다.
