...
- SamHYnix extracted expander: 위 그래프의 코스피 다리를 "삼성·삼성우·하이닉스 제외 코스피"로
  바꾼 버전 (§6-19)
- Siren Index expander: 반대로 "삼성·삼성우·하이닉스만 담은 시총가중 바스켓(SH)" 대비 내 계좌,
...
### 6-26. "Siren Index" — SH 바스켓(삼성·삼성우·하이닉스만) 대비 계좌 (2026-09-10, new1 전용)
...
- **위치**: 거래 기록 탭, `SamHYnix extracted` expander 바로 밑 `Siren Index` expander — 이름 뜻은
  "FOMO **유발** 지수"(안 담은 이 바스켓이 오르는 걸 보며 FOMO를 느낀다 — 세이렌의 노래, 돛대에
  묶여 지나침). 이름 변천: `SamsungHynix` → `FOMO Index` → `Siren Index` (전부 2026-09-10 당일).
