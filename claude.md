...
- **그래프 선 4개** (전부 소수 수익률, y축 `.1%`, anchor일 = asset_history·index_history 공통
  ...
  - hover(x unified)는 각 선의 `누적%  ·  당일%`을 같이 보여줌(지수 당일 = 전 거래일 대비,
    내 선 당일 = 직전 스냅샷 대비 `diff`). ▲ 물타기 마커는 한 번 넣었다가 뺌(사용자 요청
    2026-09-01 — 노이즈).
- **그래프 위 4줄 표(누적 / 당일)**: 코스피·코스닥·내 주식·내 계좌 각각의 최신 누적 + 당일을
  한눈에. **내 주식·내 계좌 값은 "혼합 지수"(= 보유주식의 코스피/코스닥 평가금액 비중으로
  코스피·코스닥 누적등락을 가중평균한 것)보다 높으면 빨강 / 낮으면 파랑**으로 칠함(누적은
  혼합 누적과, 당일은 혼합 당일과 비교). 색만으로 시장 대비 잘하고 있는지 즉시 보이게 한
  것(사용자 요청). `compute_index_vs_account`가 `latest` dict로 `{"코스피"/"코스닥"/"주식"/
  "계좌"/"혼합": (누적, 당일)}`을 반환하고, `혼합`은 `kospi_weight`를 줬을 때만 채워짐.
