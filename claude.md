...
- **하락 / 상승 / even 캡처** (2026-09-07, CR·엔진·합친지수 전부 폐기. 이력: RP(원점회귀
  ...
  - **누적(단순평균)**: `DC` = 하락일 c 평균, `UC` = 상승일 c 평균, `even` = even일 e 평균.
    (일별 비율의 산술평균이라 큰 하락일·작은 하락일이 1:1 — 거슬리면 `Σ내당일/Σ벤치당일`로
    바꿀 수 있음. 표기 전용이라 옆의 승률이 보정해 줌.)
  - **승률**: `ERA`(하락 방어율) = 하락일 중 `c < 1` 비율, `PCT`(상승 승률) = 상승일 중
    `c ≥ 1` 비율, `evr`(even 승률) = even일 중 `e ≥ +0.1%` 비율.
  - **내 계좌**(예수금 포함) / **내 주식**(Rs, 예수금 제외) 각각 다 계산해서 리스트에 둘 다 표기.
  - **리스트 색**: 하락(DC·ERA) 줄 = 빨강(`UP_COLOR`), 상승(UC·승률) 줄 = 파랑(`DOWN_COLOR`),
    even 줄 = 회색 — 사용자 지정, 국내 관례(상승=빨강)와 반대인 거 알고 감. 형식:
    `DC 하락 캡처 · ERA (혼합)` / `계좌 누적 0.20 · 오늘 — · 8/8` / `주식 …` (3블록 + `하락 8 ·
    상승 5 · even 2` 줄). "오늘"은 오늘이 그 바구니일 때만 값, 아니면 `—`.
  - `compute_index_vs_account` 반환: `cap = {"acct": {...}, "stock": {...}}` — 하위 키
    `dc/uc/even`(평균), `era/pct/evr`(= `(승리일수, 해당바구니일수)` 튜플), `today`/`today_bucket`.
  ...
  - **even일 로그**: `report/capture_anomalies.csv`(날짜, 벤치당일, 초과_계좌, 초과_주식)에
    ...
    분기 단위로 되짚어 보려는 기록.
  - 회귀 테스트 3개: `test_capture_down_market`(하락일 c=0.5 → DC 0.5, 방어율 6/6, UC None),
    `test_capture_up_and_down_buckets`(DC/UC 평균 + 방어율/승률 카운트),
