...
| `dividend_cache.csv` | 배당수익률 캐시, 하루 1회만 재조회(§6-15) | 종목코드, 배당수익률, 조회일 |
...
- 종목별 보유현황 카드 (비중은 예수금 포함 총자산 대비) — 종목명 옆에 배당수익률 배지(검정색,
  비중%과 같은 크기, §6-15) 표시. 카드의 WATERING 칩으로 매수/매도 내역 + 물타기 그래프 상세를 펼침
  (§6-10)
...
### 6-15. 보유종목 배당수익률 배지 (2026-09-01 도입)
- **동기**: 현재가를 네이버에서 가져오는 김에 배당수익률도 같이 보여달라는 요청. 종목별
  보유현황 카드의 종목명 옆에 비중%과 같은 크기(11px)의 검정색 텍스트로 표시.
...
- **캐싱 (하루 1회)**: 시세 실시간 API(`fetch_quotes`)와 달리 종목별로 페이지를 하나씩
  긁어야 해서(배치 API 없음), 새로고침을 누를 때마다 63개를 매번 다시 긁으면 느려질 걸
  우려해 `dividend_cache.csv`(종목코드, 배당수익률, 조회일)에 캐싱함
  (`refresh_dividend_yields()`) — 오늘 이미 조회한 종목은 건너뛰고, 새 종목/날짜가 지난
  종목만 실제로 긁는다. 실측으로는 63개 전부 새로 긁어도 약 1.6초라 우려했던 것만큼
  느리진 않았지만(2026-09-01 확인), 캐싱 설계 자체는 유지 — 종목 수가 늘거나 네이버 응답이
  느려지는 날을 대비.
- **갱신 시점**: `app.py`의 "시세 새로고침"/자동 새로고침 핸들러에서 `refresh_all_prices()`
  직후에 같이 호출됨. Fishing/Volume/Foreigner처럼 별도 버튼이 아니라 메인 시세 새로고침에
  얹은 이유는, 사용자가 "종목이름 옆에 상시 표시"를 원했기 때문(항상 보이는 카드 정보라
  전용 새로고침 버튼을 따로 두면 처음 보는 사람에게 어색함).
