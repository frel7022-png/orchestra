...
- 매칭 로직: `find_pattern_matches(decline_days, decline_pct, flat_days, flat_pct)`
  (portfolio_core.py) — **달력 날짜가 아니라 영업일(=히스토리에 쌓인 행 개수) 기준**으로
  최근 `decline_days`영업일 구간에서 고점 대비 최대 하락률(음수)을 구해서 `decline_pct`
  이상이면 1차 통과, 그중 가장 최근 `flat_days`영업일 구간의 (최고-최저)/평균 변동폭이
  `flat_pct` 이내면 최종 매치(`tail(N)`으로 행 개수 그대로 자름 — 2026-08-18에 사용자 요청으로
  달력 근사 대신 이 방식으로 바꿈). "영업일 = Fishing 새로고침을 누른 날"이라는 전제라서,
  새로고침을 자주 안 누르면 실제 달력상 기간보다 더 넓은 기간을 보는 셈이 됨(예: 40영업일
  선택해도 일주일에 한 번만 누르면 실제로는 40주치를 보는 것) — 사용자가 규칙적으로 눌러줄
  거라는 전제 위에 설계됨.
- UI: Fishing expander 안, 당일 ±3% 스크리너 밑에 구분선을 두고 별도 "패턴 검색" 서브섹션으로
  분리(사용 빈도/성격이 달라서 섞지 않음). 조건 4개 전부 `st.slider`(1단위)로 받음 — 하락 기간
  1~120영업일, 하락폭 기준 10~40%, 최근 횡보 기간 1~30영업일, 횡보 범위 기준 ±2~15%. 결과는
  종목명 + 고점대비%/최근구간 변동폭 + **미니 스파크라인**(섹터 추이 차트에 이미 쓰던 인라인
  SVG 방식 재사용, 새 라이브러리 없음)으로 보여줌 — 숫자만으론 "바닥 다지는 느낌"이 실제로
  그런지 확인하기 어려워서 그래프를 같이 넣음.
