...
### 6-13. 거래 내역 누적 요약 + Shadow Asset (2026-08-25 신설)
- **거래 기록 탭, 캘린더 위**: 누적 매수/매도 총액 + 누적 실현손익 한 줄 요약
  (`ui_transactions_tab.py`, `.trade-summary` 클래스 재사용 — §6-10에서 만든 걸 그대로 씀).
- **포트폴리오 탭, "섹터 비중 보기" 위**: **Shadow Asset** — 사용자가 만든 개념으로,
  "지금 미실현손실 상태인 종목들을 손절하지 않고 물타기로 결국 원가까지 회복시킨다"는
  전제 하의 가상 총자산. 공식: **Shadow Asset = 총자산 + 미실현손실**(예: 총자산 900만원,
  미실현손실 300만원이면 Shadow Asset = 1200만원 — "손실만 없었다면 지금 얼마였을까").
  **처음 사용자가 "총자산 + 실현손익누적"이라고 말한 적이 있었는데, 본인이 든 예시로
  역산해보니 계산이 안 맞아서(900+200=1100 ≠ 실제 의도한 1200) "미실현손실"의 오타였다는
  걸 확인하고 진행함 — 나중에 이 공식을 다시 바꾸려 한다면 이 경위를 참고할 것.**
  - **새 계산이 필요 없었음**: `asset_history.csv`의 "조정자산" 컬럼이 처음부터
    `snapshot_history(total_assets, total_assets + unrealized_loss)`로 저장되고 있던 값이라
    (원래는 §6-x 실현손익 그래프에서 "미실현손실 = 조정자산 − 총자산"을 뽑아내는 용도로만
    쓰였음), 이름만 "Shadow Asset"으로 새로 붙이고 그 컬럼을 그대로 그래프에 씀 — 과거
    데이터 소급 계산 불필요.
  - **그래프**: 파란선=총자산 추이, 빨간선=Shadow Asset(=조정자산) 추이, 회색 점선=원금
    수평선(이 앱의 다른 그래프들처럼 x축 따라 죽 이어지는 기준선 패턴 재사용, §6-10 참고).
    **색상이 이 앱의 보통 관례(빨강=상승/이익, 파랑=하락/손실)와 다르게 쓰인 것에 주의** —
    여기서는 두 선 다 "자산" 계열이라 이익/손실 의미가 아니라 그냥 "실제 vs 가상"을
    구분하는 용도로 사용자가 직접 빨강=Shadow/파랑=총자산으로 지정함. 두 선의 세로
    격차 = 그 시점의 미실현손실 크기.
  - Playwright로 실제 렌더링 확인함(2026-08-25).
