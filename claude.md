...
- **동기**: "코스피/코스닥이 2% 빠졌는데 내 자산은 0.5%만 빠졌다(혹은 5% 빠졌다)"를 매일
  ...
  `app.py` 호출부도 같이 바뀜).
- **누적 선그래프(코스피/코스닥/혼합지수/내주식/내계좌 4~5선, hover로 누적·당일 표시)는
  2026-09-05에 완전히 삭제함** — §6-17 맨 아래 "그래프 = 국면별 민감도 막대 하나만" 항목
  참고. 이유: 사용자가 궁극적으로 보려는 건 누적 CR 수치(하락장/상승장/CR)고 그 값은 아래
  표로 이미 충분히 나오는데, 선그래프는 같은 정보를 추세(모양)로만 다르게 보여줄 뿐이라
  판단(2026-09-05). 이 불릿 밑에 남아있던 "그래프 선 4개" 상세(Rs 계산식, 혼합지수 정의,
  hover 색칠 로직 등)는 지웠지만 **계산 자체(Rs, 벤치, 혼합지수)는 안 지워졌음** — 아래 표와
  국면막대가 여전히 그 값들을 쓴다. 과거 구현 경위가 필요하면 이 시점 이전 git 커밋을 볼 것.
...
- **그래프 = 국면별 민감도 막대 하나만, 캐러셀 없앰** (2026-09-05, 사용자 판단: "궁극적으로
  보려는 건 누적 CR 수치(하락장/상승장/CR)고 그래프는 매일매일 오늘 어땠나 보는 용도" — 누적
  %는 위 4줄 표로 이미 충분해서 지수 대비 계좌 선그래프(코스피/코스닥/혼합지수/내주식/내계좌
  4~5선)는 완전히 삭제, 2페이지 스와이프 캐러셀(2026-09-02 도입)도 페이지가 하나만 남아
  같이 삭제 — `st.plotly_chart()`로 직접 렌더링. `_render_iva_panel(iva, kospi_label)`에서
  `carousel_id`/`idx_hist_local` 인자도 같이 뺌(안 쓰게 됨), `components.html`/
  `_index_day_moves` import도 정리.
- **막대는 "내 주식(Rs)"이 아니라 "내 계좌" 기준** (2026-09-05, 사용자 요청 — "실제 운용
  중인 계좌 vs 지수가 매일 중요한 거지, 100% 투자했다는 이론적 가정(Rs)이 궁금한 게
  아니다"). `me["국면막대계좌"]`를 그림(이전엔 `국면막대주식`이었음). 위 "CR·내 주식"/
  "CR·내 계좌" 텍스트 요약 2줄은 그대로 유지 — Rs 기준 CR 숫자는 거기서 계속 참고 가능.
  하루 = 막대 하나, 하락일 빨강 / 상승일 파랑 / 방향 없는 날은 막대 없음. 기준선 `y=1`(대시,
  "시장과 똑같이 움직임") — 위로 갈수록 잘한 것. y축 `range=[-2.0, 3.0]` 고정.
  `go.Bar`, `marker_color`를 국면별로. **hover에 막대값만이 아니라 원본 %도 같이 표시**
  (2026-09-05, 사용자 요청 — "막대값만 보면 왜 그런지 판단이 안 된다"): 혼합지수(벤치)
  당일 / 내 계좌 당일 raw %를 customdata로 얹음 — `<b>하락장</b> 민감도막대 0.53<br>
  혼합지수 -4.16% · 내 계좌 -0.31%` 형식. plotly 범례는 끔(위 "CR·내 주식/내 계좌" 줄 +
  빨강=하락장·파랑=상승장이 범례).
...
- **위치**: 거래 기록 탭, "지수 대비 계좌" 바로 밑 `SamHYnix extracted` expander. 메인과
  **완전히 같은 렌더러**(`ui_transactions_tab._render_iva_panel()`, 5줄 표 + CR(국면막대)
  2줄 + 국면막대 그래프 1장, 2026-09-05부터 캐러셀 없음) 공유 — '코스피' 표시 라벨만
  '코스피(삼성·하이닉스 제외)'로 바뀌고 dict 키·계산은 동일. 표 라벨은 "반도체 제외"가
  아니라 **"삼성·하이닉스 제외"**로 통일(사용자 요청 2026-09-04). 표 컬럼은 **누적 / 5일 /
  당일** — "5일"은 시계열 5행 전 대비(코스피·코스닥은 5거래일, 벤치·내 주식·내 계좌는
  스냅샷 5구간, 국면막대 '5일'과 같은 기준).
- **합성 지수 계산** (`portfolio_core.synthetic_kospi_ex_bigcap(index_hist, bigcap_hist)`):
  ...
  `compute_index_vs_account`에 넘기면 코스피 다리·혼합지수·국면막대(CR)가 전부 "반도체
  제외"로 계산됨(별도 계산기 안 만듦).
...
- **읽는 법 / 주의**: 최근엔 반도체가 코스피를 떠받쳐서 "코스피 ex-반도체"가 헤드라인보다
  ...
  **초과수익(%p)과 5줄 표 빨강/파랑**은 이 버전에서 더 유리하게 나옴. 그런데 **국면막대(CR)는
  오히려 더 낮게** 나올 수 있음 — ex 지수는 `1/(1−W)≈2.1x`로 변동성이 증폭돼서, 같은 절대
  초과수익이라도 "시장 변동폭 대비 비율"로는 작아지기 때문(국면막대 정의상 정상. 판단은
  %p·표 색을 같이 볼 것).
