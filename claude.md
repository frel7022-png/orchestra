...
- **함수**: `portfolio_core.seed_engine_series(tx, initial, fee_rate, asset_hist=None)` — `_cash_by_date`와
  같은 재생 루프(§1-1)로 날짜별 (예수금, 총매입=Σ수량×평단가, 총자산, 예수금비중%)을 만든다.
  거래 있었던 날짜만. asset_hist에 없는 날 총자산은 `예수금+원가`로 근사(평가손익 제외).
- **UI**(`ui_transactions_tab.py`): plotly 3선(총매입 빨강↑ / 예수금 파랑 / 총자산 회색 점선), x-unified
  hover. 밑에 요약 2줄 — ① `총매입 A → B (+N%) · 예수금 C → D (비중 X%, 최근5 평균 Y%)`
  비중 28% 미만이면 경고색. ② `실현손익(씨앗)이 0이었으면 예수금 {D − 누적실현}원 → 실제 {D}원
  (+누적실현 보충)`.
- 회귀 테스트 `test_seed_engine_series_tracks_cash_and_cost`.
