...
- Seed Engine: 매입액↑ vs 예수금 평행 스노우볼 스코어카드 (총매입/예수금/총자산 3선 + 예수금비중,
  "실현손익이 씨앗으로 예수금을 얼마나 보충했나") (§6-27)
...
### 6-27. "Seed Engine" — 매입액↑ · 예수금 평행 스노우볼 스코어카드 (2026-09-10, new1 전용)
...
- **위치**: 거래 기록 탭, "누적 매수/매도 + 누적 실현손익" 요약 바로 밑, History Calendar 위.
- **함수**: `portfolio_core.seed_engine_series(tx, initial, fee_rate, asset_hist=None)` — `_cash_by_date`와
  ...
  `예수금+원가`로 근사.
- **UI**(`ui_transactions_tab.py`, 2026-09-10 재설계): **선 3개, 밑 요약줄 없음**. plotly x-unified hover.
  - **Cost Basis** (빨강 `UP_COLOR`, ↑여야 정상) = 총매입.
  - **W Fuel** (파랑 `DOWN_COLOR`, 평행이어야 정상) = 실제 예수금(씨앗이 채워준 것).
  - **W/o Fuel** (녹색 `NEW_COLOR` 점선, 파랑보다 더 가파르게 하락) = 씨앗(실현손익) 없었으면
    남았을 현금. **파랑−녹색 간격 = 씨앗이 채운 연료**(현재 ≈ 누적실현 43만 = 315 vs 272).
  - hover: 각 선 `{값}원 (%{총자산 대비 %} 라벨/총자산)` — "모든 그래프는 비교가 있어야 한다"(사용자).
    hover **맨 아래(W/o Fuel 줄 밑)에 `MPG {W Fuel ÷ W/o Fuel × 100}%`** — 엔진 성능("연료를 덜
    쓰는 정도"). 100%=씨앗 효과 없음, 116%=무연료 대비 탱크에 16% 더 있음. 씨앗 쌓일수록 오름
    (예수금비중이 배치로 떨어져도 MPG는 오를 수 있음 — 씨앗 효과만 분리). W/o Fuel이 0에 닿으면 ∞.
    차 연비 컨셉(2026-09-10 사용자 요청). 현재 116%(8/28 108% → 우상향).
  - **차트 밑 Burn rate 줄 (2026-09-10)**: 구간 8/19~오늘. `매입 +X · W Fuel −Y (씨앗 커버 Z%)
    · W/o Fuel −W (커버 ~0%)`. 커버율 = `(dcost − 그 선 감소분)/dcost` = 매입 확대 중 탱크를
    안 까고 씨앗으로 메운 비율. W/o Fuel은 씨앗이 없어 매입과 1:1 소진(커버 0). 둘째 줄:
    `W/o Fuel {현재}원 → 0까지 매입 여력 ~{runway}원. 그때 W Fuel 예상 ~{금액}원 = 씨앗 엔진이
    벌어준 runway`. (매입액 커지면 익절도 늘어 커버율이 오르는 게 스노우볼 정상.)
