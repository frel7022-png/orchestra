...
- **위치/형식**: 포트폴리오 탭 일일거래 요약(daily-trade-box, 요약카드 sc-bot 안) 맨 밑
  ...
  2026-09-16 요청으로 "3박자"를 전부 모음(라벨은 최소한만, Watering Detect와 같은 원칙):
  1. `총진입 {전체 사이클 수, open 포함} · 총아웃 {전량청산 완료 수}`
  2. `1회 {FA 수}({승률}%) · 물타 {MA 수} · 평균 {FA 평균 보유일수}일`
- **함수**: `portfolio_core.compute_fa_win_rate(tx)` — `_all_cycles`/`_cycle_bucket`(§6-20)을
  ...
  용도로는 날짜가 필요 없었음). 반환: win(FA)·total·n_out(전량청산 완료)·ma(물타서 전량청산)·
  win_rate·avg_days.
- **2026-09-16 실측**: 총진입 202 · 총아웃 120, 1회 79(39%) · 물타 11 · 평균 4일. 50%를
