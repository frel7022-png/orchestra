...
- **RP (relative performance) 3종 × 2줄** — 표시 이름 "RP" (2026-09-02, 최종. 원점회귀
  ...
  - **공식** `RP(창) = (내 창-누적수익 − 벤치 창-누적수익) / |벤치 창-누적수익|`. **0 = 시장과
    동일, 양수 = 시장보다 잘함, 음수 = 못함** — 상승·하락장 분기 없이 한 식. 표시는 ±3 clamp.
    예) 벤치 +2%: 내 +4%→+1, 내 +2%→0, 내 −2%→−2. 벤치 −2%: 내 −2%→0, 내 0%→+1, 내 −4%→−1.
    "시장이 X% 움직일 때 나는 그보다 몇 배 더/덜 했나"(= 시장 변동폭으로 나눈 초과수익).
  - **당일/5일/누적은 같은 식을 다른 창에서 본 것** (2026-09-04 사용자 지적으로 수정):
    당일 = 최근 1구간(양끝 차이), 5일 = 최근 `beta_window`구간(양끝 차이, 앵커 오프셋 상쇄),
    누적 = 앵커부터(표에 뜨는 계좌/주식 누적수익 값을 그대로 씀 — 사용자가 그 값으로 RP를
    읽음). **예전엔 당일 점수를 median 낸 게 5일·누적이었는데, 비율은 평균이 성립하지 않아서
    (평균(aᵢ/bᵢ) ≠ Σaᵢ/Σbᵢ) 창 누적수익 변화로 한 번만 나눔.** 최소 3구간(당일은 1),
    `|벤치 창-누적| < 0.3%`면 None(분모 폭발 방지). `portfolio_core._rp_cols`.
    회귀: `test_rp_cumulative_is_ratio_of_totals_not_average_of_dailies` — median이면 +0.5,
    올바른 누적이면 −1.5가 나오는 시나리오로 고정.
  - **왜 여기까지 왔나**: (1) 원점회귀 베타(`ΣΔ·Δ벤치/ΣΔ벤치²`)는 방향 무관이라 누적/5일이
    "방어냐 상승참여냐"를 구분 못 함. (2) "하락장 Δ주식/Δ벤치, 상승장 Δ벤치/Δ주식 뒤집기"로
    갔더니 상승장에서 내 주식이 하락하는 엣지 케이스가 지저분했음. (3) 사용자가 예시 7개로
    딱 떨어지는 `(Δ내−Δ벤치)/|Δ벤치|`로 정리. (4) 2026-09-04: 누적/5일이 median이라 "누적"의
    의미와 안 맞던 걸 창 누적 비율로 고침.
