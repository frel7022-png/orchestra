...
- Fishing: 관심종목 ±3% 스크리너, 각 줄에 순위 + 전일 대비 순위 변동(▲▼ 또는 신규 진입 NEW) +
  배당수익률 배지(종목명 옆, §6-15) 표시 (§6-6)
- Volume / Foreigner: 거래량·외국인 수급 이상치 스크리너 (§6-12)
- 종목별 보유현황 카드 (비중은 예수금 포함 총자산 대비) — 종목명 옆에 배당수익률 배지(비중%과
...
### 6-15. 배당수익률 배지 — 보유종목 + Fishing 관심종목 (2026-09-01 도입)
- **동기**: 현재가를 네이버에서 가져오는 김에 배당수익률도 같이 보여달라는 요청. 종목명
  옆에 비중%과 같은 크기(11px)의 텍스트로 "1.1% (2025.12)" 형식으로 표시 — 뒤의 괄호는
  배당기준월(아래 참고). 보유종목 카드뿐 아니라 Fishing 관심종목 줄에도 동일하게 적용
  (`ui_portfolio_tab._dividend_badge_html()` 헬퍼를 두 곳이 공유).
...
- **Fishing 줄 레이아웃 버그(수정함)**: 종목명에 `white-space:nowrap`이 없는 상태로 배당
  배지를 바로 옆에 붙였더니, 자리가 부족할 때 한글 종목명이 글자 중간에서 줄바꿈되는
  버그가 있었음(예: "SGC에너지"가 "SGC에" / "너지"로 쪼개짐 — CJK는 기본적으로 아무
  글자 사이에서나 줄바꿈이 허용되기 때문). `.updown-row .name-group`이라는 플렉스
  래퍼로 감싸서, 자리가 부족하면 종목명이 말줄임표(`text-overflow:ellipsis`)로 줄어들고
  배당 배지(`flex-shrink:0`)는 항상 온전히 보이도록 고침. **그래도 배당기준월 괄호까지
  붙이면 종목명이 너무 잘려서(2026-09-01 재확인), Fishing 줄은 결국 배당기준월 없이
  퍼센트만 표시하기로 함** — `_dividend_badge_html(code, cache, show_period=False)`.
  보유종목 카드는 배당 배지가 종목명 아래 전용 줄(바로 다음 항목 참고)이라 자리가
  넉넉해서 배당기준월을 계속 표시한다 — 두 곳이 서로 다른 인자로 같은 헬퍼를 부르니,
  이 함수를 고칠 땐 어느 쪽을 건드리는지 헷갈리지 말 것.
