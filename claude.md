...
### 1-8. `st.markdown(f"""...""", unsafe_allow_html=True)` 템플릿 — 조건부로 둘 다 비는 줄을 단독으로 두지 말 것
- Streamlit의 마크다운 파서(markdown-it)는 **공백만 있는 줄을 "빈 줄"로 취급**해서 그 지점에서
  진행 중이던 HTML 블록을 끊는다. 그 뒤에 오는, 열린 태그 없이 들여쓰기만 된 내용은 **들여쓰기
  코드블록**으로 렌더링돼 `<div>...</div>` 원문이 그대로 화면에 텍스트로 노출된다(실제로 2026-09-10
  meritz "Claude's Read"에서 겪음, 2026-09-11 new1 Holdings 카드의 `{dividend_row_html}
  {_entry_row_html}`에서 재발 — 둘 다 문자열이 비면 그 줄이 들여쓰기만 남은 "빈 줄"이 됨).
- **원인**: `{a}{b}`처럼 조건부로 빈 문자열이 될 수 있는 조각들을 **단독 줄**에 두면, 그 조각들이
  전부 비었을 때 그 줄엔 f-string의 들여쓰기(공백)만 남는다 — 이게 CommonMark 기준 "빈 줄"이라
  HTML 블록이 거기서 끊김. 해당 조각이 하나라도 있을 땐(비어있지 않을 땐) 문제없이 잘 렌더되니
  버그가 조건부로만(신규 종목·데이터 없는 종목 등에서만) 나타나서 알아채기 어렵다.
  - **주의**: 앞뒤 줄이 이미 내용이 있어도 안전하지 않다 — 마크다운은 "그 줄 자체"가 비어있는지만
    본다. `</div>` 다음 줄에 이 빈 줄이 와도 그대로 끊긴다.
- **고치는 법**: 조건부로 비는 조각을 **항상 내용이 있는 태그와 같은 줄에 이어 붙인다**
  (`{a}{b}<div class="always-here">...`처럼 — 뒤에 확실히 채워지는 태그를 바로 붙이면 그 줄은
  절대 안 빈다). 별도 줄로 빼서 보기 좋게 하고 싶은 유혹을 참을 것 — 그 줄이 비는 경우가 반드시
  생긴다는 전제로 짤 것.
- 새 HTML 조각을 카드/패널 템플릿에 추가할 때마다 "이 조각들이 전부 비면 이 줄이 완전히
  공백만 남는가?"를 확인할 것.
...
manual/                           # report/와 성격이 다름 — **살아있는 설계 문서**(2026-09-11 신설).
                                  #   report는 "그 시점의 관찰"이라 안 덮어쓰지만, manual/은 계속 고쳐 쓰는 스펙.
                                  #   지금은 오케스트라_아키텍트_매뉴얼.html 하나 — Link/CFG 아키텍처, 4분면 정의,
                                  #   실전 적용(물타기 레인지 조정) 가설, 백로그를 담음. 파일명에 버전 접미사 안 붙임
                                  #   (내용 안의 버전 이력 표로 관리) — 같은 파일에 계속 덮어써서 같은 아티팩트
                                  #   URL로 재발행. 가정이 검증/구현되면 이 문서의 상태 칩을 그때그때 갱신할 것.
...
| 파일 | 역할 | 주요 컬럼 |
|---|---|---|
| `transactions.csv` | **유일한 진실 공급원.** 모든 매수/매도 + 예수금 증감(입금/출금) 이력 | id, 날짜, 종목명, 구분(매수/매도/입금/출금), 수량, 단가, 실현손익, 메모, 정산반영 (§6-25) |
...
| `stock_sector_cache.csv` | 섹터 영구 캐시 — 2026-09-09부터 "수정 섹터" 27개 + 기타2 체계(§6-24). 153종목=수정섹터, 그 외=기타2 | 종목명, 섹터 |
...
- **보유종목 평가손익 밑에 "지수 환산 비교"** (2026-09-15 신설): `index_history` 표본 시작일
  (8/14) 종가(≈6,978)를 코스피·삼성하이닉스 제외 지수·"내 주식"(Rs, 예수금 제외 순수 가격
  변동 누적수익률) 셋의 공통 출발점으로 두고, "코스피 6,978→N · Ex-반도체 6,978→N · Orchestra
  6,978→N" 형식으로 나란히 표시(Orchestra 값은 앵커 이상이면 빨강/이하면 파랑). %보다 "지수
  포인트"로 두면 코스피 추이 그래프(거래 기록 탭)와 바로 비교가 된다는 사용자 요청. 매
  새로고침마다 `compute_index_vs_account`/`synthetic_kospi_ex_bigcap`으로 다시 계산돼 자동
  갱신됨 — 별도 저장 없음. `idx_compare_html`이 조건부로 비면 `<div class="summary-grid">`와
  같은 줄에 이어붙여서 §1-8 "빈 줄" 버그를 피함.
- 자산 추이 그래프
- Pit Stop expander (⛽, 평가손익 그리드 ↔ Today's Take 사이): Cost Basis↑(빨강)·Refill=예수금(녹색)·
  No Refill=씨앗없을시(파랑)·Surplus=연료여분(진노랑, 우측 ±30% 축, +빨강/−파랑). 씨앗이
  예수금을 얼마나 아껴주나 (§6-27)
- 섹터 비중: 도넛차트 + 섹터별 막대(0~40% 고정 스케일, 목표선 표시, 전일 대비 변화) + 섹터 클릭 시
  일별 추이(0~40% y축, raw 값 표시)
- Up/Down: 완전 매도한 종목 중 마지막 매도가 대비 변한 것 추적 — **DOWN ±2% / UP ±3%**
  (2026-09-10, 하락 쪽을 더 촘촘히). 전용 새로고침 버튼, 자동 실행 안 됨
- Fishing: 관심종목 ±3% 스크리너, 각 줄에 순위 + 전일 대비 순위 변동(▲▼ 또는 신규 진입 NEW)
  표시 (§6-6). 배당수익률 배지는 한 줄짜리 목록이라 종목명이 잘려서 결국 안 씀(§6-15).
  그 종목이 **Up/Down 결과에도 걸려 있으면**(매도 후 DOWN 2%/UP 3% 이탈) Fishing에서 그
  **종목명을 파랑**으로 표시 — Up/Down 새로고침을 눌러 결과가 있을 때만 (2026-09-10)
- Bench: 한 번도 매수한 적 없는 관심종목(소외종목) — Fishing 밑, Volume 위 (§6-23)
...
- **이익 종목만**(손익 ≥ 0) 손익 셀에 두 가지 추가 (2026-09-10): (1) 손익 금액 위에 작은
  회색 배지로 **최초 진입일 + 보유 거래일수** `8/25(15일)` — 최초 진입일 = 현재 보유 사이클
  첫 매수일(`get_holding_trade_points` 첫 매수행, 물타기 제외), 거래일수는 `index_history`의
  실제 거래일로 셈(KR 휴장일 반영, 8/14 이전 구간만 월~금 근사 — 2026-08엔 평일 휴장 없어
  이 근사가 정확). (2) 손익 금액 옆에 **세금 차감 후 실현액 병기** `+2,000(+1,985)` — 괄호 =
  `손익 − 평가금액×fee_rate`(지금 전량 매도 시 순액, §6-4), 둘 다 빨강. CSS `.cell .entry`(app.py).
...
- SamHYnix extracted expander: 위 그래프의 코스피 다리를 "삼성·삼성우·하이닉스 제외 코스피"로
  바꾼 버전 (§6-19)
- Sour Grapes expander: 반대로 "삼성·삼성우·하이닉스만 담은 시총가중 바스켓(SH)" 대비 내 계좌,
...
- 거래 내역 캘린더 — 월 네비게이션(◀ YYYY년 M월 ▶) 밑에 **그 달의 실현손익 합계**(그 달
  매도 실현손익 Σ, 양수 빨강/음수 파랑). 달을 넘기면 그 달 값으로 바뀜 (2026-09-10)
...
- **6번째 재발 (2026-09-17) — 이번엔 asset_history가 아니라 portfolio_data.csv "표시용
  현재가" 자체가 며칠씩 멈춰있었음**: 위 5번째 재발 수정의 주석("화면에 보이는 현재가는
  여전히 앱의 라이브 새로고침이 담당")이 전제로 깔았던 게 실제로는 안 지켜지고 있었다 —
  `ingest_daily.py`는 원래 "코드 미확인 종목이 있을 때만" 세션이 수동으로
  `refresh_all_prices()`를 돌리는 구조였는데(§6-2 문서 그대로), 코드가 이미 다 있는
  평범한 날엔 이 스텝 자체가 통째로 스킵된다. 그러면 `portfolio_data.csv`에 커밋되는
  현재가/등락률은 "마지막으로 라이브 새로고침이 실제로 성공했던 시점"에 그대로 멈춘 채
  커밋되고, 그 이후 며칠이고 다음 코드-미확인 사태가 없으면 아무도 안 건드린다.
  **배포 서버의 라이브 새로고침(§4 "앱을 열 때 자동 새로고침")은 git에 안 올라가므로
  (§6-1)**, 이 stale 값이 계속 새 커밋에 실려 재배포될 때마다 "그 시점으로 되돌아가는"
  것처럼 보인다 — 오늘처럼 짧은 시간에 커밋을 여러 번 하는 날엔 특히 눈에 띔. 실제로
  겪음: GS리테일이 9/15 17:20 시점 값(등락률 -5.79%)으로 이틀 넘게 멈춰있었는데, §6-33
  Today's Alarm이 이 stale 등락률을 "오늘 -5.8% 급락"으로 표시함(사용자가 "차트 보면
  그렇게 안 빠졌는데?"라고 지적해서 발견 — 실제로 라이브 재조회하니 -0.21%였음).
  **수정**: `ingest_daily.py`가 코드 보충 여부와 무관하게 **매 반영마다 무조건**
  `refresh_all_prices()`를 한 번 돌려 커밋하도록 변경 — 이제 committed 스냅샷이 "최대
  하루" 이상 stale해질 수 없다. `compute_metrics_at_close` 기반 스냅샷 계산(위 5번째
  재발 수정)은 이미 그 앞 단계에서 끝나 있으므로, 여기서 표시용 현재가만 갱신해도
  asset_history/sector_history엔 영향 없다.
- **7번째 재발 (2026-09-17) — 이번엔 index_history/bigcap_history, 그리고 원인이 처음으로
  "로직 복제"였음**: `_close_on`(확정 종가 우선, 없으면 실시간 폴백) 헬퍼가 처음부터
  `ingest_daily.py`의 **로컬 클로저**로 new1·meritz 두 스크립트에 각각 인라인 복제돼
  있었다 — "같은 함수인데 왜 파일이 두 개냐"는 사용자 지적(2026-09-17)으로 드러남.
  로직 자체는 두 파일이 우연히 계속 동일했지만, **실행 결과(커밋되는 값)는 실행 시점에
  달려있어서 복제 여부와 무관하게 위험**했다 — 실제로 meritz의 `index_history[9/16]`이
  확정 종가(6,717.97·815.98)가 아니라 그날 장중 어느 시점 스냅(6,653.21·805.7, 65p·10p
  차이)으로 커밋된 채 방치돼, 그 이후 모든 "당일" 코스피/코스닥/혼합지수 계산이 실제보다
  크게 부풀려짐(사용자가 "코스피 0.75%인데 왜 혼합지수가 1.90%냐"고 지적해서 발견 —
  1.90 ≈ 0.9×0.75+1.22 같은 우연한 합산처럼 보였지만 실제 원인은 계산식이 아니라
  베이스라인 날짜 자체가 틀렸던 것). **new1의 `bigcap_history[9/14~9/16]`도 확정
  종가와 최대 24,000원(SK하이닉스 9/15) 어긋나 있었음** — 두 레포 다 그날그날 언제
  ingest를 돌렸느냐에 따라 각자 다른 장중 순간을 찍어와서 서서히 벌어진 것으로 보임.
  SamHynix extracted 합성지수는 대형주 비중을 `1/(1−W)≈2.1x`로 증폭하는 구조라
  (§6-19) 이런 오차가 실제 화면에서는 훨씬 크게 보인다.
  **수정**: `_close_on` 로직을 `portfolio_core.confirmed_close_or_live(code, trade_date,
  fallback)` 공유 함수로 통합(new1·meritz 양쪽 `portfolio_core.py`에 동일하게 추가) —
  `ingest_daily.py`는 이제 `_close_on = core.confirmed_close_or_live`로 별칭만 두고
  직접 호출. 오염됐던 index_history[9/16]·bigcap_history[9/14~16]는 두 레포 모두
  `fetch_daily_price_history`로 재조회한 확정 종가로 덮어씀. **로직이 여러 파일에
  복제돼 있으면, "지금은 똑같아 보여도" 미래에 파일마다 다른 실행 시점에 걸려 서로 다른
  결과를 커밋하게 될 잠재 위험이 항상 있다** — 이번처럼 "왜 파일이 둘이냐"는 질문이
  나오면 웬만하면 공유 함수로 옮길 것.
...
- **`fetch_investor_flow` API 완전 교체 (2026-09-14) — 비공식 스크레이핑 단일 실패점이 실제로
  터진 사례**: 9/10·9/11 cron 로그를 보니 `investor_flow` 조회가 **이틀 연속 180종목 전부
  실패**(price_history/market_flow는 정상 — §6-9의 방어적 구조가 의도대로 피해를 격리함)했는데
  워크플로우 자체는 "success"로 찍혀서 겉으로는 티가 안 났음(사용자가 "습관이 여전히 도움이
  되냐"고 물어봐서 실제로 열어보다가 발견). 원인: 네이버가 `finance.naver.com/item/*` 종목
  페이지 전체를 Next.js 클라이언트 렌더링 앱으로 개편(2026-09-10 전후) — 이 함수가 긁던 정적
  HTML 표(`/item/frgn.naver`, `summary="...순매매 거래량..."` 테이블)가 최초 HTML 응답에서
  통째로 사라짐(페이지는 200 OK로 응답하지만 `<table>` 자체가 없음). 이 함수 자신의 옛
  docstring이 "네이버가 페이지 구조를 바꾸면 조용히 깨질 수 있다"고 경고했던 게 실제로 일어난
  사례. **수정**: 그 Next.js 페이지가 내부적으로 부르는 모바일 API
  (`m.stock.naver.com/api/stock/{code}/integration`의 `dealTrendInfos` JSON — §6-15가 배당락일
  조사 때 이미 한 번 확인해둔 엔드포인트)로 완전히 교체. HTML 파싱보다 JSON 스키마가 안정적이고,
  반환 계약(날짜/거래량/기관순매수/외국인순매수/외국인보유율 dict 리스트)은 그대로라 호출부
  (`db_fetch_daily_prices.py`)는 무수정. **다만 이 엔드포인트는 최근 5영업일치만 준다**(옛
  HTML 표는 ~20일치) — 일일 cron은 하루 한 행만 필요해서 문제없지만, cron이 5영업일 넘게
  멈췄다 재개되면 그 사이 공백은 이 함수만으로 못 채움(수동 재백필 필요). 세션이 로컬에서
  `python db_fetch_daily_prices.py` 한 번 실행해 9/10·9/11 공백을 즉시 메움(900행 upsert,
  두 날짜 다 180/180 확인). 회귀 테스트 `test_fetch_investor_flow_parses_real_json_structure`를
  새 JSON 스키마로 교체. **`fetch_dividend_yield`(§6-15)도 같은 페이지 개편의 영향권**(
  `/item/coinfo.naver`도 Next.js로 바뀐 것 확인함) — "최초 1회만" 캐시라 기존 캐시 종목은
  영향 없지만 신규 종목의 배당 조회는 조용히 계속 실패할 것. 아직 안 고침(2026-09-14 기준),
  다음에 다룰 것. `fetch_market_flow`(`/sise/*` 페이지)는 이번 개편 영향 없음 확인함(9/10·9/11
  둘 다 정상 upsert).
...
- **그래프** (`_render_holding_detail()`, plotly, `ui_transactions_tab.py`의 자산추이 차트와
  ...
  - **"현재가" 선 = DB(Supabase price_history) 일별 종가 그대로**(2026-09-11 갱신).
    예전엔 최초매입일→오늘 두 점을 직선으로만 이었는데(보유종목엔 연속 시세 히스토리가 없어서
    두 점만 가능했음), 사용자가 "그 사이 등락 없이 꾸준히 내려온 것처럼 보인다"고 지적 —
    실제로는 오르내렸는데 직선이 그걸 숨겼던 것. `get_stock_price_history_db(종목코드, sb_url,
    sb_key)`(신설, `load_watchlist_history_db`처럼 전체를 다 받지 않고 그 종목코드 하나만
    필터링) 로 최초매입일~오늘 구간 실제 종가를 가져와 그린다 — 첫/끝 점은 실제 체결가·
    실시간가로 고정(DB 종가와 하루 이틀 차이 날 수 있어서). DB에 그 종목 데이터가 없으면(막
    편입돼 아직 watchlist cron이 못 돈 신규 종목 등) 예전처럼 두 점 직선으로 폴백 — **네이버를
    그때그때 낱개로 조회하지 않고 DB만 조회**한다. 세션당 종목코드 1회만 조회
    (`st.session_state["holding_price_hist_cache"]`, 카드를 열어둔 채 다른 위젯을 눌러
    rerun돼도 재조회 안 함).
...
- **그래프 선 4개** (전부 소수 수익률, y축 `.1%`, anchor일 = asset_history·index_history 공통
  ...
  - **내 주식(검정, 굵게)** = `Rs(t)`: 보유주식을 100% 투자했다고 환산한 누적수익 →
    ...
    0이라 비교 기준이 맞음).
  - **내 계좌(회색 점선)** = `총자산(t)/최초자본 − 1` 을 **anchor일(8/14) = 0 으로 리베이스**한 값
    (`(1+r_t)/(1+r_0)−1`, 2026-09-08 사용자 요청 — 코스피/코스닥/혼합지수/내 주식이 전부 8/14
    기준이라 "8/14부터 시장 대비"가 정합이 되게). 절대 계좌수익(+1.14% 류)은 요약카드·
    "vs 최초자본"에 그대로 있음. 예수금이 눌러주는 "완충된" 선. 이 선과 `Rs` 선 사이 간격 =
    예수금 쿠션 효과. DC/UC(계좌 기준)는 `계좌당일` diff가 `1/(1+r0)`배만 스케일돼 사실상 불변.
...
- **하락 / 상승 / even 캡처** (2026-09-07, CR·엔진·합친지수 전부 폐기. 이력: RP(원점회귀
  ...
  - **UI = 바구니마다 미니 표 3개** (`_cap_tbl`, 2026-09-07 사용자 요청으로 텍스트줄 → 표):
    ...
    DC 표 = 파랑(`DOWN_COLOR`, 하락일 방어), UC 표 = 빨강(`UP_COLOR`, 상승일 참여), even 표 = 회색
    (2026-09-08 스왑 — 국내 관례에 맞춤. 이전엔 DC 빨강/UC 파랑이었음). 표 3개 밑에 `하락 8 · 상승 6 · even 0` 줄.
...
- **그래프 = 2장 스와이프 캐러셀** (`_render_iva_panel`, `components.html` iframe, `#{carousel_id}`
  ...
  - **2장** = `[하락/상승/even 캡처 표 3개(HTML) + 일별 캡처 막대]`. 막대 = 하루 = 막대 하나,
    y = 그날 캡처 `c`, 하락일(DC) 파랑 / 상승일(UC) 빨강, **even일은 그래프에서 완전히 제외**(단위 %p라
...
- **위치/잠금 (2026-09-11부터 UI는 §6-28 "Link"로 교체됨 — 이 절의 계산 함수 자체는 그대로
  살아있음, 나중에 필요해지면 재사용)**: 포트폴리오 탭 Foreigner expander 밑
  `외인 매수 → 이후 주가 (실험)` expander였음. **이 패널만 추가 비밀번호로 가림**(사용자 지정
  값, 지금은 `ui_portfolio_tab.LINK_PASSWORD`로 이름만 바뀜) — `check_password()`의 앱 전체
  로그인과 별개. 프로젝트용이라 아직 완성 전이라서 걸어둔 소프트 게이트.
...
- **UI 컨트롤 (구 버전, 2026-09-11 이후 비활성)**: `기준`(평균/전일) + `문턱`(+0.2/+0.3/+0.5 %p)
  라디오. 표 2개(호라이즌별 평균 시장초과·상승률·vs 평상시 / 최근 이벤트별 T+1~T+3) —
  `_render_fop_panel()`이었으나 이 함수는 삭제됨(§6-28 참고).
...
- **버킷 (사이클 = 진입~전량매도 단위, `portfolio_core._all_cycles` + `_cycle_bucket`)**:
  - **FA** (First in, All out) = 1매수 → 부분매도 없이 한 방에 전량매도.
  - **MO** (Multiple Out) = **부분매도가 1회라도 있으면 무조건 MO** (우선순위 최상 — 물타기
    여부·매도 여부 무관. 물타면서 부분익절해도 MO, 나중에 전량 나가도 MO). "나온 것만" 집계.
  - **MA** (Multiple in, All out) = 2+매수(물타기) → 부분매도 없이 한 방에 전량매도.
...
- **보관 정책 변경 — 누적 이력 → 최신 1건만 (2026-09-17, 사용자 지시)**: 원래는 CSV에
  매일 계속 append해서 이력을 다 쌓아두고(UI는 어차피 마지막 것만 보여줌) — 이제는
  **"하루 지나면 전날 것은 없앤다"**로 바뀜: 새 코멘트를 쓸 때 그 전에 있던 행을 전부
  지우고 새 행 하나만 남긴다(누적 이력 자체를 CSV에도 안 남김). 2026-09-17에 9/10·9/11·
  9/14·9/15 기존 4건을 전부 지우고 new1·meritz 둘 다 빈 파일(헤더만)로 초기화 — 그날
  Claude's Read 자체도 "안 써도 된다"는 지시로 건너뜀. **다음에 코멘트를 쓸 땐 새 행 1개만
  추가하고, 그 전 행(있다면)은 먼저 지울 것** — append만 하던 예전 루틴으로 되돌아가지 말 것.
- **루틴 (하루 1회, 오후 3:30 KST 이후 = 장 마감 후)**: 매매일지 반영 + `sync_both_accounts.py`
  ...
  - **"3:30 이후"는 "16:00 이후"로 더 보수적으로 지킬 것 (2026-09-14 실제로 겪음)**: 같은 날
    `daily_stats.py`를 세 번(13:41 장중, 16:20 장마감 직후, 최종 매매일지 반영 후) 돌렸더니
    혼합지수 당일이 -1.92% → -2.71% → -2.72%로 계속 바뀌었다. 원인은 두 겹 — (1) 장중에
    돌리면 당연히 그 순간까지의 부분적인 움직임만 잡힘(가장 큰 원인, §1-8 아님 §6-2 5번째
    재발과 같은 "장중 스냅" 문제의 daily_stats 버전). (2) **장마감(15:30) 직후~16:00 사이엔
    두 시세 소스가 서로 살짝 어긋난다** — 실시간 시세(`fetch_quotes`, 시간외단일가 15:40~16:00
    체결이 섞여 들어갈 수 있음)와 일별시세 확정 종가(`fetch_daily_price_history`, 네이버 쪽
    정리에 몇 분 지연이 있는 것으로 보임)가 짧은 시간 다른 값을 줄 수 있다(실측: 같은 날
    삼성전자가 248,500원 vs 249,000원, SK하이닉스가 1,697,000원 vs 1,695,000원으로 갈림).
    **결론: 매매일지가 장중에 먼저 들어와도 반영 자체는 바로 해도 되지만, `daily_stats.py`
    실행 + Claude's Read 코멘트 확정은 16:00 KST 이후로 미룰 것** — 그 전에 이미 한 번
    돌렸다면 16:00 이후 다시 돌려서 수치를 갱신하고 코멘트를 다시 쓸 것(그날 안에서는 여러 번
    덮어써도 됨, §1-2와 같은 원칙).
...
- **동기**: watchlist(현재 179개) 중 실제로 한 번도 안 산 종목만 따로 보면서 "내가 계속 지나친
  종목이 지금 어떻게 움직이나"를 추적. 포트폴리오 탭 **Fishing expander 바로 밑 / Volume 위**에
  `st.expander("Bench")`, 아코디언 패턴.
- **대상**: `load_watchlist()` 종목명 − `tx[구분=="매수"]`에 나온 종목명 = 미보유(never-bought).
  "한 번이라도 샀으면" 제외(전량매도해서 지금 보유 0이어도 제외). **2026-09-09 기준 77개.**
- **데이터**: 별도 조회 안 하고 **Fishing의 새로고침이 채운 `st.session_state["fishing_prices"]`를
  그대로 재사용**(refresh_watchlist_prices가 이미 179개 전부 조회함). 비어있으면 "Fishing에서
  새로고침을 먼저 눌러주세요" 캡션만.
- **행 포맷**: Fishing과 같은 `.updown-row` 레이아웃 — `순위. 종목명  누적%  전일%`.
  **±3% 필터 없음(전부 표시), 순위변동 배지(▲▼/NEW) 없음, 단가 컬럼 없음**(사용자 지시).
  누적% = (최근가−최초가)/최초가, 최초가 = Fishing과 동일(`get_first_day_prices_db`, DB 최초
  관측일 종가). 전일% = 네이버 `change_pct`.
- **기준(누적/전일)·방향(DOWN/UP) 라디오**: Fishing과 동일하게 둠. 여기선 정렬 키로만 쓰임.
- **정렬**: `(매수횟수 asc, 선택 기준·방향 값, 현재가 desc)`.
  - 매수횟수 = 그 종목 `구분=="매수"` 행 수(물타기·재매수 전부 누적, 사이클 무시). **지금은
    미보유만 다루니 전부 0 → 사실상 무의미**, 나중 단계용.
  - 선택 기준·방향: 누적/DOWN이면 많이 빠진 순(오름차순), 누적/UP이면 많이 오른 순(내림차순),
    전일도 동일. **지금 단계에선 이게 실질 정렬**.
  - 현재가 desc = "산 횟수 같으면 단가 높은 게 우선" 타이브레이크(화면엔 안 보임).
- **표시 개수**: 미보유 종목 수 **> 50이면 전부**, **≤ 50이면 상위 30개만**. 지금 77개라 전부.
  - **미보유가 50 이하로 줄었을 때(또는 결국 다 한 번씩 샀을 때) 모집단을 "미보유 종목"에서
    "watchlist 전체(매수횟수 적은 순)"로 넓힐지는 아직 미정** — 사용자가 "그건 나중에 생각"
    이라고 보류(2026-09-09). 지금 코드는 미보유 풀 안에서 30개로만 자름.
- **변경 범위**: `ui_portfolio_tab.py`의 render_portfolio_tab에 expander 블록 추가만. 새 파일·새 함수·
  새 DB 조회 없음. meritz 미적용(관심종목 스크리너 자체가 meritz엔 없음).

### 6-24. 섹터 체계 전면 교체 — "수정 섹터" 27개 + 기타2 (2026-09-09, new1 전용)
- **동기**: 기존 섹터가 세분류(디스플레이장비·리츠/부동산·섬유의류…)에 `SECTOR_GROUP_MAP`으로
  이상한 병합(인터넷→반도체, 화장품→소비재)까지 껴서 지저분했음. 사용자가 watchlist 153종목을
  **27개 카테고리로 직접 재분류한 엑셀**(`temporary/피싱_섹터.xlsx`, 컬럼 `종목명/섹터/수정 섹터/종목코드`)을
  주면서 "종목별로 새로 짝 지어준 것", 이걸 전면 적용.
- **27개 카테고리**: 건설·게임·금속철강·금융·기타·레저·렌탈·로봇·미용·바이오·반도체·식품·에너지·
  엔터·의료기기·의류·이차전지·인터넷·자동차·전기전자·제약·조선·통신·포장재·해운·화장품·화학.
  + **기타2** = 153에 없는 종목 전부(§6-16으로 나중 편입된 보유 18개 포함, "the26"). 사용자 지시로
  하나로 묶음 — 나중에 개별 매핑 주면 그때 반영.
- **적용한 곳** (한 번에):
  - `stock_sector_cache.csv` — 153종목=수정섹터, 그 외(watchlist 나머지 26 + 레거시)=기타2. 192행.
  - `portfolio_data.csv` + `checkpoint_holdings.csv` — 보유 73종목 전부 `portfolio_core.fix_holding_sector()`
    루프로 갱신(§6-14 되돌아감 버그 방지 — 세 파일 동시). 55개=수정섹터, 18개(the26)=기타2.
  - Supabase `watchlist.sector` — 179종목 전부 PATCH(153=수정섹터, 26=기타2). cron
    (`db_fetch_daily_prices.py`)은 sector를 안 건드리므로 안 덮임.
  - `portfolio_core.SECTOR_GROUP_MAP` — 옛 병합(인터넷→반도체 등) 전부 제거, 레거시→신체계 흡수
    5개만 남김(반도체소재/반도체장비→반도체, 건자재→건설, 엔터테인먼트→엔터, 렌탈서비스→렌탈).
    `group_sector()`는 27개(+기타2)를 그대로 통과 → Sectors 도넛/막대/드릴다운이 새 체계로 자동 재집계.
  - `constants.py` — `SECTOR_PALETTE` 30색으로 확장(섹터 수 늘어서), `SECTOR_TARGETS`에서 "소비재"
    제거(해당 종목 사라짐), "식품 30%"만 남김. **새 27개 기준 목표는 사용자가 정해주면 추가.**
  - `portfolio_core`의 섹터 기본값 "미분류" → **"기타2"** 전부 교체(load_watchlist / load_holdings /
    apply_transaction 신규행 / compute_metrics / DB 조인 fallback). 이제 "미분류"라는 섹터는 없음.
- **앞으로 들어오는 종목 규칙 (사용자 지시)**: 매매일지로 새 보유종목이 생기면 —
  **이름이 `피싱_섹터.xlsx`의 153에 있으면 그 수정섹터, 없으면 기타2**. `apply_transaction`이
  `stock_sector_cache.csv`(=153 매핑 보유 중)를 보고 없으면 "기타2"로 자동 배정하므로 **세션이
  따로 손댈 필요 없음**. 153에 있는데 새로 편입되는 종목(예전에 안 사던 걸 사기 시작)도 캐시에
  이미 수정섹터가 들어있어 자동으로 맞게 잡힌다.
- **sector_history.csv 불연속**: 옛 행은 옛 그룹명(유통 물류·의료바이오·소비재…), 새 스냅샷부터
  새 27개. Sectors "일별 추이"에서 이름이 바뀐 섹터(의료기기 등)는 추이가 오늘부터 새로 시작됨 —
  §6-2의 "히스토리 lock-step" 부류 이슈, 수용.
- **엑셀 원본**: `temporary/피싱_섹터.xlsx`(로컬 전용, git 안 올라감). 27개 canonical 리스트는 위에.

### 6-25. 예수금 이자(예탁금 이용료) 반영 — 구분 "입금"/"출금" 거래 (2026-09-09, new1 전용)
- **동기**: 앱 예수금이 메리츠 앱 예수금보다 만원 적게 나오던 걸 추적해보니(총매입·총평가는 일치),
  메리츠가 **월 1회 지급하는 예탁금 이용료(예수금 이자)**가 transactions엔 없어서 재생 결과에
  안 잡힌 것. 재생 검산도 딱 맞음: `초기자본 − Σ매수 + Σ매도 − Σ매도세 = 예수금`.
- **해결**: `transactions.csv`에 **`구분="입금"`(또는 "출금") 행**을 한 줄 추가.
  `종목명=""`, `수량=1`, `단가=금액`, `메모="예탁금이용료"`. `apply_transaction`이 이 분기를
  가장 먼저 처리 — `state["cash"] += 금액`(출금은 −), 보유종목·실현손익·사이클엔 **전혀 영향 없음**.
  메모가 `DAILY_IMPORT_TAG`("일일매매일지")가 아니라서 `ingest_daily.py`가 그 날짜 매매일지를
  재반영해도 이 행은 안 지워진다.
- **replay를 타므로** 체크포인트/전체재계산에도 영구 반영되고, `_cash_by_date`(§6-17 지수 대비
  계좌)에도 자동으로 들어가서 "최초자본 대비 손익"엔 이익으로 잡힌다(원금 아님).
- **`_all_cycles`**(§6-20 P&L Actions)는 맨 앞에서 `구분.isin(["매수","매도"])`로 걸러 입금/출금
  행을 무시한다. 거래 기록 탭 캘린더는 입금/출금 줄을 `예탁금이용료  +10,000원` 형태로 별도 표시.
- **운영**: 이자 들어올 때마다 사용자가 금액·날짜 알려주면 세션이 한 줄 append → 전체 재계산 →
  git commit. 2026-09-09에 10,000원(2026-09-09자) 1건 반영해서 예수금을 메리츠 앱과 맞춤.
...
### 6-26. "Sour Grapes" — SH 바스켓(삼성·삼성우·하이닉스만) 대비 계좌 (2026-09-10, new1 전용)
...
- **위치**: 거래 기록 탭, `SamHYnix extracted` expander 바로 밑 `Sour Grapes` expander — 이름 뜻은
  "여우와 신포도"(안 담은 이 바스켓이 오르는 걸 보며 FOMO 유발 — 못 딴 포도가 시다). 이름 변천:
  `SamsungHynix` → `FOMO Index` → `Siren Index` → `Sour Grapes` (전부 2026-09-10 당일).
...
### 6-27. "Pit Stop" — 매입액↑ · 예수금 평행 스노우볼 스코어카드 (2026-09-10, new1 전용)
- **이름 변천 (전부 2026-09-10)**: 개념명 "Seed Engine" → 토글/패널명 `⛽ Pit Stop`(레이싱 피트스톱
  = 주유+정비). 선 이름 `총매입/예수금/무연료예수금/MPG` → `Cost Basis / Refill / No Refill / Surplus`
  ("성능 좋다"가 아니라 "연료가 쌓인다"는 개념이라 MPG→Surplus).
- **동기 (사용자 전략 프레임)**: 이 계좌에서 **실현손익은 "이익"이 아니라 "씨앗"** — 기회(제대로 된
  하락)가 왔을 때 규모 있게 물탈 총알을 미리 쟁이는 것. **예수금은 놀고 있는 현금이 아니라
  "저장된 기회"**. 그래서 이 계좌의 스코어카드는 NAV 증감이 아니라 **"매입액은 계속 커지는데
...
- **위치**: **포트폴리오 탭, 평가손익 그리드 박스 아래 · Today's Take 위 (그 둘 사이)**. 요약 카드를
  `st.container(key="summary_card_wrap")` 안에서 3조각으로 나눠 렌더 — (A) `.summary-box.sc-top`
  (평가손익+그리드) / (B) `st.container(key="seed_engine_wrap")` + `st.expander("⛽ Pit Stop")` /
...
- **No Refill 재정의 (2026-09-16)**: 원래 정의(예수금−누적실현손익, "매도할 때마다 항상 본전에
  ...
  약하다는 지적(2026-09-16 리포트 `report/2026-09-16_정교한_방패.html` §01)에서 출발 —
  **"실현손익 대신, 실제로 팔아서 현금화했던 그 돈이 벤치(삼성·하이닉스 제외 혼합지수)를
  따라갔다면 남았을 예수금"**으로 교체: `No Refill(t) = 예수금(t) − 누적실현손익(t) +
  그날까지 누적된 벤치 기준 가상실현손익(t)`.
  - **핵심 원칙 — "예수금 대 예수금" 비교여야 한다(사용자가 두 차례 시행착오 끝에 확정,
    2026-09-16)**: 벤치 반사실은 **전량매도 완료된 사이클에만, 그 사이클이 실제로
    투입했던 금액(buy_amt)과 실제 보유기간(최초매수일~매도일)에만** 적용한다.
    - **1차 시도(기각)**: 사이클별 buy_amt만 반사실에 넣는 "보수적" 안 → 사용자가 "보통
      사람이면 초기자본(1000만원)을 다 넣었을 것, 250만원 현금으로 남긴 것도 전략"이라며
      **초기자본 전체**로 확장하자고 정정.
    - **2차 시도(기각)**: `초기자본 × 벤치누적수익률`을 통째로 예수금에서 빼는 안 → 실측
      해보니 Surplus가 +70%까지 튀었고, 사용자가 재검토 끝에 **한 번도 안 굴린 현금(약
      250만원, 1/4)까지 시장에 노출된 것처럼 취급**하는 오류를 지적. 사용자의 비유: "가상의
      상대(passive 추종자)도 76만원 손실이 전부 **미실현**으로 자기 계좌(주식) 안에 있지
      현금에 있는 게 아니다 — 나도 마찬가지로, 아직 안 판 물량의 미실현손익은 예수금에
      닿으면 안 되고, **실제로 현금화한(=실현한) 금액만** 벤치와 비교해야 진짜 '예수금 대
      예수금' 비교가 된다."
    - **최종(현재 구현)**: 결국 1차 시도의 "사이클별 buy_amt 스케일"이 맞았다 — "보수적 근사"가
      아니라 현금 비교를 정확하게 만드는 유일한 방법이었던 것. 초기자본 전체를 쓰면 안 굴린
      현금까지 이중으로 벌점을 받고, 총매입(현재 원가)으로 스케일해도 미실현(아직 안 판 것)
      효과가 현금에 잘못 섞여 들어간다 — **오직 "실현된" 사이클만, 그 사이클의 실제 투입액과
      보유기간만** 정확히 대응시켜야 함.
  - `portfolio_core.virtual_realized_cum_by_close_date(tx, bench_cum)` 신설 — `_all_cycles`(§6-20)로
    매도된 사이클을 뽑아, 각 사이클마다 `buy_amt × ((1+벤치(매도일))/(1+벤치(최초매수일)) − 1)`을
    계산해 매도일 기준으로 누적한 날짜→누적가상실현손익 dict를 반환. 사이클 내부 부분매도
    타이밍까지는 안 쪼개고 매도일에 그 사이클의 가상손익 전체를 한 번에 반영(§6-20
    `pl_first_pct`와 같은 수준의 근사). 열린(아직 안 판) 사이클은 실제 실현손익과 마찬가지로
    제외.
  - `portfolio_core.blended_benchmark_cum(index_cum, kospi_weight)` — `compute_index_vs_account`
  ...
  - `bench_cum`이 없거나 매도된 사이클이 없으면 가상실현손익이 전부 0 → 자연스럽게 예전 정의
    (실현손익 0% 가정)로 축소. `bench_cum=None`으로 부르면(기존 호출부·회귀테스트 그대로)
    완전히 예전 동작과 동일.
  - **2026-09-16 실측**: 매도된 사이클들의 벤치 기준 가상실현손익 누적 −104,095원(실제
    누적실현손익 +457,182원과 대비 — 즉 내 실제 매매가 벤치 대비 약 +561,000원의 알파를
...
- **동기 (§6-18 포프의 다음 단계)**: "외인이 사면 오른다"류 전체 상관관계를 보려는 게 아니다 —
  2026-09-11 리포트(`report/2026-09-11_조용한_손_시끄러운_주가.html`)로 관심종목 178개 전체
  상관이 거의 0(0.05 안팎)임을 이미 확인함. 대신 **원래 같은 방향이어야 할 가격과 외인비중이
  이번엔 크게 반대로 벌어진 "엉뚱한 놈"만 골라서, 한 달 정도 시간을 두고 그게 어떻게 되는지
  지켜보는 것**이 목적. 사용자 가설(2026-09-11): 삼성전자 같은 초고유동성 대형주는 외인 물량에
  단타·차익거래·지수 리밸런싱이 섞여 신호가 묻히지만, 오케스트라처럼 저유동성·저관심 종목군은
  외인이 들어오는 이유가 좁아서(대체로 펀더멘털 판단) 신호 대 잡음비가 오히려 나을 수 있다 —
  이 패널이 성립하는 전제.
...
- **감시목록 — `link_watch_log.csv`** (컬럼: 플래그일,종목코드,종목명,기준일,기준가,기준외인비중,
  ...
  **§1-7 원칙대로 배포된 앱의 실시간 UI에서는 이 파일에 쓰지 않는다** — 사용자가 채팅으로
  "이 종목 감시목록에 넣어줘"라고 하면 **세션이 `add_link_watch_entry()`를 스크립트로 실행하고
  git commit/push**한다(배포 서버 로컬 디스크 쓰기는 재배포 때 사라짐, §1-5 — Fishing
  `watchlist.csv`가 `import_watchlist.py`로 세션이 갱신하는 것과 같은 패턴). `기준가`/`기준외인비중`은
...
- **추세(누적 vs 최근3일) — 2026-09-11 신설**: 구간은 누적(기준일pp) 하나로만 정해지는데, 그
  안에서 "지금도 그 방향으로 가는 중"인지 "최근 며칠 새 이미 꺾였는지"는 다른 정보임 — 사용자
  예시: 대한항공이 기준일 대비 외인비중 +2.21%p라 "진행형"처럼 보여도, 최근 3거래일은 오히려
  빠지는 중이면 "아직 덜 먹었다"가 아니라 "슬슬 나가는 초입"일 수 있음. `compute_foreign_flags`가
  `최근3일pp`(오늘−3거래일전)도 계산, `compute_link_candidates`가 `추세`("진행중"=누적·최근
  부호 같음 / "꺾임"=반대)로 판정해서 "꺾임"인 행은 `_mag`에 0.35를 곱해(완전 배제는 아님,
  3일 창은 노이즈가 있어서) 그 구간 안에서 순위만 낮춤. UI(다이버전스·진행형 목록)에서 "꺾임"
  행은 우측 detail 칸이 `score N` 대신 빨간 `꺾임 최근±N%p`로 바뀜.
- **UI (`_render_link_panel(ph, fh, live_quotes, refresh_fn, T)`, `ui_portfolio_tab.py`)**:
  ...
  ② **다이버전스** — 구간="다이버전스" 상위 8개(구간 내 `_mag` 내림차순, 추세="꺾임"이면 감점).
  최우선 후보 목록.
...
- **기간 세분화는 보류**: "1개월/2개월 등 기간별로 나눠서 보기"는 지금(DB 약 한 달치)은 의미가
  ...
  쌓이면 그때 재검토.
- **다음에 논의 — "이탈→다이버전스" 조기 감지 함수 (2026-09-11 사용자 요청, 착수 보류)**:
  사용자 요청 원문: "173개 중, 누적으로 계속 주가랑 외인 비중이 떨어지고 있었는데, 누적 주가
  떨어지는 게 잔잔해지면서 외인 비중이 다시 3~5일씩 조금씩 쌓이는 종목을 골라내는 거지." —
  즉 "이탈" 구간(P<0,dF<0 누적) 종목 중에서 **최근 며칠 새 두 가지가 동시에 일어나는 것**만
  추려내는 함수: (1) 외인비중 추세가 이미 "꺾임"으로 반전(최근3일pp>0, 위 항목의 로직 재사용
  가능) + (2) **가격 하락 속도 자체가 둔화**(새 지표 필요 — 예: 누적 하루평균 하락률(P/관측일수)
  대비 최근 N일 하루평균 하락률이 더 완만해졌는지 비교). (2)는 아직 구현 안 한 새로운 지표라
  설계가 더 필요함 — **사용자 지시: "그 펑션을 만들면 나에게 먼저 얘기해봐" — 구현 전 설계를
  먼저 공유하고 컨펌받을 것.** 이건 "진행형→차익실현" 조기감지(위 항목)의 정확히 반대 방향이자,
  §6-28 CFG 개념의 "④다이버전스 조기 포착"에 해당 — 완성되면 지금의 "이탈" 구간에서 갈라져
  나오는 새 카테고리(가칭 "반전 조짐")가 될 것.
- **회귀 테스트**: `test_compute_link_candidates_classifies_four_quadrants`(4분면 분류 + 구간
  우선순위 정렬 확인), `test_compute_link_candidates_demotes_reversed_recent_trend`(누적·최근
  추세 반대면 "꺾임"으로 판정하고 순위 감점되는지),
  `test_compute_link_candidates_uses_price_history_start_as_baseline`
...
- **동기**: Link(§6-28)가 178개 관심종목 전체에서 "엉뚱한 놈"을 찾는 거라면, 이건 그 신호를
  **내가 이미 물타는 중이거나, 판 지 얼마 안 됐거나, Fishing에서 하락으로 걸린** — 즉 이미
  ...
  실행 가능성이 높다 — 워터링 판단, 재진입 판단, 신규 편입 판단에 바로 붙는 종목들이라.
- **셋 다 하락(DOWN) 모집단만** — 상승 중인 종목에 외국인이 느는 건 이 패널의 관심사가 아님
  (2026-09-16 사용자 지시 — "결국 떨어지는데 외국인이 사는 걸 보려는 거니까").
  - **Convoy** = Watering Detect(§ 포트폴리오 탭) 모집단 — 물타기 중(현재 사이클 2회 이상
  ...
  - **Encore** = Up/Down 모집단(처음엔 Up/Down expander 안에 뒀다가, "세 구획을 한곳에
    ...
    `updown_results`를 그대로 읽어 재사용, 별도 새로고침 없음) — 완전 매도한 종목 중 매도가
...
- **핵심 함수**: `portfolio_core.foreign_pct_change_since(flow_hist, name, since_date)` —
  ...
  최초 관측일로 고정되지 않고 **호출부가 원하는 날짜**를 받는다는 게 차이 — Convoy/Encore처럼
  "내 거래일" 기준이 필요한 경우를 위해 신설. investor_flow 커버리지가 since_date보다 늦게
  시작하면(그 종목이 나중에 편입돼 cron이 늦게 붙은 경우 등) **그 이후 가장 이른 값으로
  대체**한다(Fishing 최초가와 같은 폴백 원칙, 2026-09-16 사용자가 명시적으로 확정: "포리너
  디비가 그거보다 날짜가 뒤라면 최대한 앞쪽 날짜로 가져오면 되겠다"). 세 구획이 전부 이 함수
  하나만 재사용 — 계산 로직 중복 없음.
- **표시 기준**: 외인비중 변화 **+3%p 이상만** 보여주되, 그런 종목이 하나도 없으면(특히
...
### 6-30. FA 승률 — Fishing/Up-Down 기반 진입 판단의 정확도 (2026-09-16)
- **동기**: "물타기가 성공했다/실패했다"가 아니라 **"애초에 물 안 타고 한 번에 끝났으면
  좋았을 판단이었나"**를 재는 지표. 사용자 정의: 한 번 사서 한 번에 전량매도(FA, §6-20)한
  사이클만 성공, 물을 탔든(MA)·나눠 팔았든(MO)·아직 보유 중이든(HOLD, 물타기 여부 무관)
  전부 실패로 센다 — "처음 판단이 정확했으면 물 안 타고 한 번에 끝났을 것"이라는 전제.
- **위치/형식**: 포트폴리오 탭 일일거래 요약(daily-trade-box, 요약카드 sc-bot 안) 맨 밑
  두 줄 — "P&L Actions(§6-20)에 이미 있는 지표지만 중요하니 매일 앞에서 보고 싶다"는
  2026-09-16 요청으로 전부 모음. 라벨은 **§6-20과 같은 버킷명(FA/MA/MO)을 그대로 씀**
  (처음엔 "1회/물타/나눔"으로 풀어썼다가 "이미 이름이 있으니 그걸 쓰자"는 요청으로 통일):
  1. `Total {전량매도 완료 수}/{전체 사이클 수, open 포함} ({비율}%)`
  2. `FA {수}({승률}%) · MA {수}({비율}%) · MO {나눠팔아서 전량매도한 수}({비율}%)`
  — FA+MA+MO(닫힌 것만) 합이 Total의 분자와 정확히 일치(2026-09-16 사용자가 "79+11=90인데
  왜 120이냐"고 지적해서 MO_closed를 추가로 노출 — 30이 나눠팔기로 매도된 몫이었음). 괄호
  안 %는 셋 다 **전체 사이클 수(total) 대비 비율**(FA만 있던 걸 MA/MO에도 확장, 2026-09-16
  "MA MO 퍼센트 하도록 하자").
  **평균 보유일수는 화면 표시에서 뺌**(2026-09-16, "저 평균 4일은 지워줘") — `compute_fa_win_rate`
  자체의 `avg_days` 필드는 그대로 남겨둠(당장 딴 데서 안 쓰지만 계산 자체는 가벼움).
- **함수**: `portfolio_core.compute_fa_win_rate(tx)` — `_all_cycles`/`_cycle_bucket`(§6-20)을
  재사용하지만 holdings 없이 카운트만 뽑는 가벼운 버전(§6-20 compute_pnl_actions과는 별도
  함수). `_all_cycles`에 `first_buy_date`/`close_date` 필드를 추가해야 했음(원래 §6-20
  용도로는 날짜가 필요 없었음). 반환: win(FA)·total·n_out(전량매도 완료)·ma·mo_closed·
  win_rate·avg_days. **검산**: win+ma+mo_closed == n_out.
- **2026-09-16 실측**: Total 120/202 (59%), FA 79(39%) · MA 11 · MO 30 · 평균 4일. 50%를
  하나의 참고선으로 두고 지켜보기로 함(사용자: "지금 하락장 7% 진행 중이니 50%만 돼도 제
  기능 발휘하는 걸로 볼 수 있다") — 지금은 그 아래. 사용자 자체 평가: 표본 202건이 전부
  이번 하락장(한 달 -5~7%) 안에서 나온 거라 애초에 FA가 나오기 불리한 환경 — 39%도 "낮게
...
- **① Price Brackets — 가격대별 구성**: `portfolio_core.price_bracket_distribution(tx)` —
  ...
  - UI: `app.py`의 기존 `.sector-bar-*` CSS 클래스(섹터 비중 막대와 동일 컴포넌트, 2026-09-16
    ...
    상대 스케일(고정 스케일 아님, 섹터 목표 마커 같은 부가 요소 없음). 항상 펼쳐진 채로
    바로 보임(expander 아님).
...
- **①-보조. 지금 계좌 분포(파랑, 같은 날 추가)**: `portfolio_core.
  ...
  **지금 보유 중인 종목**의 분포를 파란 막대로 나란히 보여준다. 사용자 지적: "Price
  ...
  - `_price_bracket_label(px)`로 구간 판정 로직을 `price_bracket_distribution`과 공유
    (한 곳에만 정의, 복제 없음).
  - **레이아웃 — 빨강/파랑 전체를 따로 두 블록으로 안 보여주고, 구간마다 짝지어서
    (사용자 지시, 같은 날 정정: "둘이 비교되게 빨간 막대 밑에 파란 막대가 낫지 않을까")**:
    `_render_bracket_bars_paired(dist_sold, dist_held, T)`가 "1만원 이하" 빨강 막대 →
    바로 그 밑 "1만원 이하" 파랑 막대 → 다음 구간… 순으로 렌더. **두 분포를 하나의
    `max_pct`(둘 중 큰 값)로 같이 정규화**해서 막대 길이가 색끼리도 바로 비교되게 함
    (따로 정규화하면 서로 다른 총량인데 같은 길이로 보일 수 있어서).
  - **범례 한 줄만 예외적으로 유지**(사용자 지시: "밑에 빨간거는 뭐 파란거는 뭐 하나
    정도는 써주는 게" — §6-32에서 방금 없앤 "설명 캡션 전부 제거" 원칙과 달리, 색
    구분은 텍스트 없이는 뜻을 알 수 없어서 순수 범례 한 줄("빨강 매도 이력 · 파랑
    현재 보유")만 예외로 남김 — 기능 설명이 아니라 범례라 [[feedback_no_ui_explainer_captions]]
    위반은 아니라고 판단).
...
- **② Top Traded — 최다 매도 종목 (순수 매매 기록만, 현재가 안 씀)**:
  ...
  확정 "가격순")으로 전부 반환. 반환 컬럼: 종목명, 매도횟수, 최초진입가, 최초진입일,
  누적실현손익, 누적실현손익률(=그 종목의 매도 완료 사이클 전부의 실현손익 합 / 매수총액 합).
  - **`min_abs_realized_pct`(기본 1.0%p) 미만인 종목은 아예 뺀다** — 사용자 지시: "월덱스·
    에코플라스틱·필옵틱스처럼 처음에 계좌 종목 정리하려고 위아래로 0.대% 나온 것들은
    제외"(실측: 월덱스 −0.71%, 에코플라스틱 +0.30%, 필옵틱스 +0.64% — 전부 하루~이틀
    만에 되판 1회성 정리 거래, 실제 매매 판단으로 보기 어려움). 73개 → 63개 종목으로
    감소(2026-09-16 실측).
  - **UI — expander + top 20 + "더보기" 버튼**: Price Brackets 바로 밑
    `st.expander("Top Traded")`("항상 해오던" 이 앱의 아코디언 패턴, Up/Down·Fishing과 같은
    UX). 처음엔 상위 20개만 카드로 보여주고(`_TOP_TRADED_PAGE_SIZE=20`), 더 있으면 "더보기"
    버튼으로 `st.session_state["top_traded_show_n"]`을 20씩 늘려 다시 렌더 — 새 계산도
    네트워크 조회도 전혀 없는 순수 슬라이싱이라 새로고침 버튼 자체가 필요 없다.
  - 카드 형식: `종목명  N회` / `누적실현손익 ±원 (평균 손익률 ±%)` — 라벨 없이 숫자 위주
    (Watering Detect와 같은 원칙).
  - **2026-09-16 실측 상위**: NAVER(8회, 누적실현 +46,462원/+2.7%), 삼성전자(5회,
    +18,362원/+1.2%), 서진시스템(5회, +9,616원/+5.3%), 실리콘투(4회, +11,637원/+3.6%).
- **버려진 방향 — "현재가 대비 승패"(Selection Index, 같은 날 만들었다가 같은 날 폐기)**:
  한때 "최초진입가 대비 지금 가격"까지 비교해서 "그 매도 결정이 사후적으로 옳았나"를
  종목당 승/패로 매기는 기능(Selection Index)과, 그 승패 강도를 트리밍 평균으로 보여주는
  기능까지 만들었었다 — 갭(=누적실현손익률−가격변화율)이 임계값을 넘으면 승/패, 그 사이는
  집계에서 제외, 벤치마크(혼합지수)는 "양쪽에서 빼도 수학적으로 상쇄돼 그대로 나온다"는
  걸 확인하고 개별 판정에서 뺌, "지수 하락률로 종목 breadth를 역산하자"는 후속 보정안도
  통계적 근거가 없다는 문제 제기에 기각. **그런데 사용자가 결정적 결함을 지적**: "현재가"가
  매일 움직이는 값이라 판정 자체가 계속 흔들린다 — "종목이 170개 넘다 보니 한동안 안 산
  것도 있는데, 지금은 승리지만 시간이 흐르면 패배가 될 수도 있고 그 반대도 마찬가지"
  (§6-18/§6-28의 "포프"류 forward-return 지표처럼 고정 시점을 못 박는 방식으로 고칠
  수도 있었으나) — 최종적으로 "어차피 Up/Down이 '판 뒤 가격이 어떻게 됐는지'를 전담하고
  있어 여기서 또 현재가를 쓸 이유가 없다"(사용자)고 판단해 **Selection Index·평균 강도
  비교·현재가 관련 코드 전부(`selection_index()` 함수, `_all_cycles`의 `close_price`
  필드, `top_traded_stocks`의 `current_prices`/기준가/가격변화 컬럼, UI의 하루 1회
  고정 새로고침 메커니즘까지) 롤백**하고 순수 매매 기록(진입가·매도횟수·실현손익)만
  남김. "괜히 일만 더 만드는 거 같다"(사용자) — 짧게 살았다 사라진 기능이지만, 왜
  안 되는지 검증한 과정 자체는 §6-18 포프의 "틀린 것도 명시적으로 로그" 원칙대로 여기
  남겨둔다.
- **용어 정리 — "청산" → "매도" (2026-09-16 당일, 사용자 지시)**: "청산은 너무 네거티브한
  단어"라 이 앱 전체(코드 주석·docstring·UI 문구·claude.md)에서 "청산"을 "매도"/"전량매도"로
  바꿈. 뜻은 그대로(사이클이 완전히 끝난 것 = 전량매도) — 그냥 표현만 순화.
