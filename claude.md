...
- 지수 대비 계좌 그래프: 코스피/코스닥 vs 내 주식수익(예수금 제외)/계좌수익, 각 선의 누적+당일
  4줄 표(내 주식·계좌는 보유비중 반영 혼합지수 대비 빨강/파랑) + CR(국면별 민감도 막대) (§6-17)
...
- **CR (capture ratio) — 국면별 민감도 막대 + 요약** (2026-09-04, RP를 통째로 갈아엎음.
  RP 이력: 원점회귀 베타 → 방향 일관 점수 → "정규화 알파" `(Δ내−Δ벤치)/|Δ벤치|` → 이번에 국면
  막대로 폐기. 폐기 이유: "0 = 시장과 동일"이 하락장이냐 상승장이냐에 따라 좋음/나쁨이 뒤집혀
  직관과 안 맞았고, 누적/5일이 median이라 "누적"이라는 말과도 안 맞았음).
  - **하루(스냅샷 구간)마다 막대 하나.** 그날 벤치(혼합지수)가 내렸으면 "얼마나 안 따라 빠졌나
    (방어)", 올랐으면 "얼마나 따라 올랐나(참여)":
    - 하락일(`벤치당일 < 0`): 막대 = `1 − 내당일/벤치당일`. 0 = 시장만큼 빠짐, 1 = 하나도 안
      빠짐, 2 = 빠진 만큼 오히려 벎, 음수 = 시장보다 더 빠짐.
    - 상승일(`벤치당일 > 0`): 막대 = `내당일/벤치당일`. 0 = 하나도 못 벎, 1 = 시장만큼, >1 =
      더 벎, 음수 = 상승장인데 손해.
    - **둘 다 높을수록 좋음. 기준선 y = 1 = 시장과 똑같이 움직임.**
  - **요약 숫자 2줄** (`CR·내 주식 (혼합)  오늘 방어 +0.80 · 하락장 +0.55 · 상승장 +0.78 ·
    CR 1.42 · 5일 +0.61` / `CR·내 계좌 (혼합) ...`):
    - **하락장** = 하락일 막대들의 단순평균, **상승장** = 상승일 막대들의 단순평균.
    - **CR = 상승장 ÷ 하락장.** 1 = 대칭("똔똔"), >1 = 우호적 비대칭(상승 참여 > 하락 방어).
      하락장 평균이 0 근처(`|d_avg| ≤ 1e-6`)면 None.
    - **5일** = 최근 `beta_window`(=5)개 막대의 단순평균, 국면 무관(예: -1 -3 0 -3 -1 → 0).
    - **오늘** = 마지막 구간 막대. 라벨은 그날 국면이 하락이면 "방어", 상승이면 "참여".
    - 사용자 판단(2026-09-04): 비율(막대)을 그냥 평균 냄. 73종목이 섹터별 ~670만원으로 분산돼
      있어 벤치~0인데 계좌만 튀는 날이 거의 없고, 나더라도 그 구간 5일치 하나 뭉개질 뿐 +
      이벤트로 기억. 차트를 이쁘게 만들려는 게 아님.
  - **EPS 가드**: `|벤치당일| < 0.001`(시장 사실상 무변동)이면 그날은 막대 없음 + 이상치로
    `report/capture_anomalies.csv`(날짜, 벤치당일, 내당일, 사유)에 누적 기록. 기록은 매 렌더링이
    아니라 `app.py`의 "시세 새로고침" 핸들러에서만(`append_capture_anomalies`, 코스피 단독
    기준으로 충분 — 무변동 판정이라). 월단위로 수동 검토해서 필요하면 그 날 CR을 후보에서 제외.
  - **내 주식** = Δ주식수익(Rs, 예수금 제외) 기준(아래 막대 그래프가 그리는 값),
    **내 계좌** = Δ계좌수익(예수금 포함) 기준. 색: 하락장 = 빨강(`UP_COLOR`), 상승장 =
    파랑(`DOWN_COLOR`) — 그래프 막대색과 맞춤(요약 줄도 동일).
  - `compute_index_vs_account` 반환 키: `cap_today/cap_down/cap_up/cap_cr/cap_5d` +
    `acct_cap_*` + `cap_anomalies`(내 주식 기준 이상치 리스트) + `sensitivity_basis`
    (`"혼합"`/`"코스피"`, 어느 벤치 기준인지). `me` 시계열 컬럼: `국면막대주식` / `국면막대계좌`
    (float, 막대 없는 날은 NaN) + `국면`(`"하락"`/`"상승"`/`""`). 계산은 내부 `_cap_cols()` 하나.
    회귀 테스트 3개: `test_capture_down_market`(하락장 0.5배 → 막대 0.5, 상승장·CR None),
    `test_capture_up_down_and_cr`(하락장·상승장 평균 + CR = 상승÷하락 + 5일 평균),
    `test_capture_eps_guard_logs_anomaly`(무변동 날 막대 없음 + `cap_anomalies` 기록).
- **그래프 = 2장짜리 스와이프 캐러셀** (사용자 요청 2026-09-02, "옆으로 밀면 다음, 밑에 점"):
  1장 = 지수 대비 계좌(위 4선), 2장 = **국면별 민감도 막대 그래프**(하루 = 막대 하나, 하락일
  빨강 / 상승일 파랑 / 방향 없는 날은 막대 없음). 기준선 `y=1`(대시, "시장과 똑같이 움직임") —
  위로 갈수록 잘한 것. y축 `range=[-2.0, 3.0]` 고정. 누적·5일 선은 없앰(사용자 요청 —
  "이어지는 선이 아닌 막대선"). `compute_index_vs_account`가 `me`에 `국면막대주식`/`국면` 컬럼을
  채워서 그림(`go.Bar`, `marker_color`를 국면별로). 캐러셀은 `st.components.v1.html`(iframe)에
  두 `fig.to_html()` 조각을 CSS `scroll-snap` 트랙으로 넣고, 밑에 점(`.dot`)은 작은 JS(스크롤
  위치 → `.on` 토글)로 현재 장 표시 + `Plotly.relayout`으로 iframe 폭 맞춤. 두 그래프 다 plotly
  범례는 끔(1장은 위 4줄 표가, 2장은 위 "CR·내 주식/내 계좌" 줄 + 빨강=하락장·파랑=상승장이
  범례). **주의**: 로컬에서 `streamlit run`을 `pkill -f`로 못 죽이고
...
- **함수** (`portfolio_core.py`): `load/save/snapshot_index_history`, `append_capture_anomalies`,
  ...
  (내부 `_cap_cols`), `load/update_market_cache`, `fetch_stock_markets`, `refresh_market_cache`.
  회귀 테스트 7개 (`tests/test_portfolio_core.py`: 예수금 비중으로 de-lever되는지, 추가매수
  flow가 Rs에서 빠지는지, 지수 누적, latest의 누적/당일, 혼합 벤치·기준 라벨, 국면막대 3종
  (하락장/상승장·CR·EPS 가드), 시장 캐시 왕복).
...
- **위치**: 거래 기록 탭, "지수 대비 계좌" 바로 밑 `SamHYnix extracted` expander. 메인과
  **완전히 같은 렌더러**(`ui_transactions_tab._render_iva_panel()`, 5줄 표 + RP 2줄 +
  스와이프 캐러셀) 공유 — '코스피' 표시 라벨만 '코스피(삼성·하이닉스 제외)'로 바뀌고
  dict 키·계산은 동일. 캐러셀 DOM id는 `cwrap`/`cwrap_ex`로 분리. 표 라벨은 "반도체 제외"가
  아니라 **"삼성·하이닉스 제외"**로 통일(사용자 요청 2026-09-04). 표 컬럼은 **누적 / 5일 /
  당일** — "5일"은 시계열 5행 전 대비(코스피·코스닥은 5거래일, 벤치·내 주식·내 계좌는
  스냅샷 5구간, RP '5일'과 같은 기준).
