...
- 지수 대비 계좌 그래프: 코스피/코스닥 누적등락 vs 내 주식수익(예수금 제외)/계좌수익, 지수
  하락일 물타기 ▲ 마커 + 물타기 성적표 expander (§6-17)
...
### 6-17. 지수 대비 계좌 그래프 + 물타기 성적표 (2026-09-01 도입)
- **동기**: "코스피/코스닥이 2% 빠졌는데 내 자산은 0.5%만 빠졌다(혹은 5% 빠졌다)"를 매일
  추적하면서 종목 포트폴리오를 재점검하고, **물타기가 실제로 미래 자산을 방어했는지** 눈으로
  보려는 것. 거래 기록 탭 "실현손익 그래프" 바로 아래에 같은 스타일의 plotly 그래프로 있음
  (`ui_transactions_tab.py`, `render_transactions_tab`에 `holdings` 인자 추가됨 → `app.py`
  호출부도 같이 바뀜).
- **그래프 선 4개** (전부 소수 수익률, y축 `.1%`):
  - **코스피 / 코스닥**: anchor일(asset_history·index_history 공통 시작일, 현재 2026-08-14)
    종가 대비 누적등락, 0 중심. 연한 참조선(앰버/틸).
  - **내 주식(예수금 제외)** = `Rs(t)`: 보유주식을 100% 투자했다고 환산한 누적수익 →
    ...
    0이라 비교 기준이 맞음).
  - **내 계좌(예수금 포함)** = `총자산(t)/최초자본 − 1`: 앱 요약카드의 그 값. 예수금 40%가
    눌러주는 "완충된" 선(점선). 이 선과 `Rs` 선 사이 간격 = 예수금 쿠션 효과.
  - **▲ 물타기 마커**: 그날 코스피 **또는** 코스닥이 −1% 이하로 빠진 날의 추가매수(현재 보유
    사이클에서 첫 매수 이후의 매수 = §6-10 `_current_cycle_transactions` 기준). 날짜별로 묶어
    한 점, hover에 그날 지수 등락 + 종목명. 지수 안 빠진 날의 일상적 추가매수(전체 77건 중
    대부분)는 노이즈라 제외 — `get_dip_watering_events(drop=-0.01)`.
- **캡션**: `누적 초과수익 X%p (내 주식 vs 코스피)` + `최근 민감도 ±N.NN (코스피 −1% → 내
