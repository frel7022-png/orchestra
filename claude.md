...
- **하락 / 상승 / even 캡처** (2026-09-07, CR·엔진·합친지수 전부 폐기. 이력: RP(원점회귀
  ...
  - **UI = 바구니마다 미니 표 3개** (`_cap_tbl`, 2026-09-07 사용자 요청으로 텍스트줄 → 표):
    ...
    관례(상승=빨강)와 반대인 거 알고 감. 표 3개 밑에 `하락 8 · 상승 6 · even 0` 줄. 위 5줄
    지수 표 바로 밑, 캐러셀 위.
...
- **그래프 = 2장짜리 스와이프 캐러셀** (사용자 요청 2026-09-02, "옆으로 밀면 다음, 밑에 점"):
  1장 = 지수 대비 계좌(코스피/코스닥/혼합지수/내주식/내계좌 누적 선), 2장 = **일별 캡처 막대**
  — 하루 = 막대 하나, y = 그날 캡처 `c`, 하락일 빨강 / 상승일 파랑, **even일은 그래프에서
  완전히 제외**(단위가 %p라 캡처 축과 안 섞임 — 값은 리스트로만). `y=1`에 얇은 수평선("시장과
  똑같이 움직임") — 빨강이 1 아래 = 방어 잘함, 파랑이 1 위 = 참여 잘함. y축 `range=[-1.5, 3.0]`.
  그래프는 **내 계좌만**(리스트엔 둘 다). hover에 그날 혼합지수·내 계좌 당일 % 원본도.
  `me`의 `캡처계좌`/`바구니`로 그림(`go.Bar`, even·미분류는 아예 x에서 빼서 gap). 캐러셀은
  `st.components.v1.html`(iframe)에 두 `fig.to_html()` 조각을 CSS `scroll-snap` 트랙 + 밑에
  점(`.dot`) JS(스크롤 위치 → `.on`) + `Plotly.relayout`으로 폭 맞춤. plotly 범례는 둘 다 끔.
  **주의**: 로컬 `streamlit run` 중복 실행되면(Windows) 옛 코드가 계속 뜸 — `netstat -ano |
  grep :PORT` + `taskkill //F //PID`로 정리. Playwright 시각 검증은 `channel="chrome"` +
...
- **위치**: 거래 기록 탭, "지수 대비 계좌" 바로 밑 `SamHYnix extracted` expander. 메인과
  **완전히 같은 렌더러**(`ui_transactions_tab._render_iva_panel()`, 5줄 표 + 하락/상승/even
  캡처 리스트 + 스와이프 캐러셀) 공유 — '코스피' 표시 라벨만 '코스피(삼성·하이닉스 제외)'로
  바뀌고 dict 키·계산은 동일. 캐러셀 DOM id는 `cwrap`/`cwrap_ex`로 분리. 표 라벨은
...
- **"코스피 추이" 라인차트** (2026-09-04 추가): "지수 대비 계좌" 패널 바로 밑. 일반 코스피
  ...
  그 시점의 **전일 대비 등락률(%)**. `SamHYnix extracted` expander는 그 밑. 캐러셀(§6-17)은
  점 클릭 + 좌우 화살표 키로도 넘어감(PC는 스와이프 불가).
