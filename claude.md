...
- **민감도 3종 × 2줄** (2026-09-02 재설계 — 원점회귀 베타 → 방향 일관 점수):
  `민감도·내 주식 (혼합)  누적 0.49 · 5일 0.58 · 당일 0.58` / `민감도·내 계좌 (혼합)  ...`.
  - **왜 바꿨나**: 예전엔 `ΣΔ·Δ벤치/ΣΔ벤치²` 원점회귀 베타였는데, 오른 날·빠진 날을 섞어
    회귀하면 "누적/5일"이 방어를 잘했는지 상승장을 잘 탔는지 구분이 안 됐음(베타는 방향
    무관 — 낮은 베타는 하락장엔 좋지만 상승장엔 못 따라간 것). 사용자 지적으로 **구간마다
    방향에 따라 식을 바꿔** "1이면 시장 동일, <1이면 시장 이김, >1이면 시장에 짐"이
    상승·하락장 모두 성립하게 함:
    - **하락장**(Δ벤치<0): 점수 = `Δ주식 / Δ벤치` — 음수(내가 오름)=최고, 0=완전방어, <1=선방, >1=더 빠짐
    - **상승장**(Δ벤치>0), 내주식도 상승: 점수 = `Δ벤치 / Δ주식` (뒤집음) — 5%장에 2.5%만 오르면 2.0(반타작)
    - **상승장인데 내주식 하락**: 점수 = 상한 `SCORE_CAP=3.0` ("올라야 하는데 빠짐=최악")
    - 점수는 `[-1.0, 3.0]`으로 clamp. **음수는 하락장에서만** 나옴.
  - **누적/5일** = 구간별 점수의 **중앙값**(median — 벤치가 0 근처인 날 값 폭발을 이상치로
    흘림). 최소 3구간 필요, 없으면 None. **당일** = 마지막 1구간 점수.
  - **내 주식** = Δ주식수익(Rs, 예수금 제외) 기준 — 아래 민감도 그래프가 그리는 값.
    이 줄의 **누적=검정 / 5일=녹색 / 당일=빨강** 색이 곧 그래프 선 색 = 범례 겸용.
  - **내 계좌** = Δ계좌수익(예수금 포함) 기준. 예수금 완충 때문에 시장과 덜 붙음
    (하락장선 방어 유리, 상승장선 불리). 사용자 요청(2026-09-02).
  - **판정 원칙**: "잘하냐"는 **초과수익(내 주식 − 벤치, 표 위 줄)**이 맡고, 민감도 3종은
    "어떻게 이겼나(방어형? 공격형?)"를 보조 설명. `sensitivity_basis`는 `"혼합"`/`"코스피"`.
  - `compute_index_vs_account` 반환: `sens_all/recent/today`(내 주식, `sensitivity` =
    `sens_recent` 하위호환) + `acct_sens_all/recent/today`(내 계좌). `me`엔 시계열 컬럼
...
- **그래프 = 2장짜리 스와이프 캐러셀** (사용자 요청 2026-09-02, "옆으로 밀면 다음, 밑에 점"):
  ...
  y축 `range=[-1.1, 3.2]` 고정, 기준선 `y=1`(대시, "시장 동일") — 1 아래=시장 이김,
  1 위=시장에 짐, 음수=하락장 역행. `compute_index_vs_account`가 `me`에
