...
- **위치**: 포트폴리오 탭 일일거래 요약(daily-trade-box, 요약카드 sc-bot 안) 맨 밑에
  `N/M (W%) 평균일수 D일` 형식으로 표시 — 라벨 없이 숫자만(Watering Detect와 같은 원칙).
  N=FA 사이클 수, M=전체 사이클 수(진행 중 포함), 평균일수=FA 사이클들의 (첫 매수일~전량
  매도일) 평균(달력일).
- **함수**: `portfolio_core.compute_fa_win_rate(tx)` — `_all_cycles`/`_cycle_bucket`(§6-20)을
  그대로 재사용. `_all_cycles`에 `first_buy_date`/`close_date` 필드를 추가해야 했음(원래
  §6-20 용도로는 날짜가 필요 없었음).
- **2026-09-16 실측**: 79/202 (39%), 평균 4일. 50%를 하나의 참고선으로 두고 지켜보기로 함
  (사용자: "지금 하락장 7% 진행 중이니 50%만 돼도 제 기능 발휘하는 걸로 볼 수 있다") —
  지금은 그 아래.
