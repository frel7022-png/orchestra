...
- **UI**: `ui_portfolio_tab.py`의 "종목별 보유현황" 카드 밑에 버튼("▼ 매수/매도 내역 · 물타기
  그래프")을 추가 — 누르면 그 카드 바로 밑에 상세가 펼쳐짐(`st.session_state.holding_detail_
  open`에 종목코드 저장, 한 번에 하나만 펼쳐짐 — 아코디언 방식). 기존 카드 HTML(`.stock-card`)
  자체는 그대로 두고, 그 아래에 별도 버튼만 추가한 것 — 카드 디자인을 버튼으로 통째로 바꾸진
  않음.
