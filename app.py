...
<style>
    ...
    .updown-row.flow-row .detail {{ width:66px; }}
    ...
    .foreign-tag {{ font-size:11px; margin-left:0; white-space:nowrap; }}
    .badge-sep {{ font-size:11px; margin:0 6px; color:{T['muted2']}; }}
    ...
    /* 요약 카드를 3조각(평가손익+그리드 / Pit Stop 토글 / Today's Take+Claude's Read)으로
       나눠 렌더하고 CSS로 틈을 없애 카드 하나처럼 보이게 (§6-27). */
    [class*="st-key-summary_card_wrap"] [data-testid="stVerticalBlock"] {{ gap:0 !important; }}
    [class*="st-key-summary_card_wrap"] [data-testid="stElementContainer"],
    [class*="st-key-summary_card_wrap"] [data-testid="stMarkdown"],
    [class*="st-key-summary_card_wrap"] [data-testid="stMarkdownContainer"] {{ margin:0 !important; }}
    .summary-box.sc-top {{ margin-bottom:0; border-bottom-left-radius:0; border-bottom-right-radius:0; padding-bottom:12px; }}
    .summary-box.sc-bot {{ margin-top:0; border-top:none; border-top-left-radius:0; border-top-right-radius:0; padding-top:0; }}
    /* Pit Stop 토글 — Claude's Read식 작은 글씨, 카드 안에 이어붙게(테두리 X, 배경 = 카드색) */
    [class*="st-key-seed_engine_wrap"] {{ background:{T['card']}; border-left:1px solid {T['border']}; border-right:1px solid {T['border']}; padding:0 8px; }}
    [class*="st-key-seed_engine_wrap"] details {{ border:none !important; background:transparent !important; }}
    [class*="st-key-seed_engine_wrap"] summary {{ padding:6px 12px 2px !important; }}
    [class*="st-key-seed_engine_wrap"] summary p {{ font-size:13px !important; font-weight:600 !important; color:{T['muted2']} !important; }}
    [class*="st-key-seed_engine_wrap"] summary svg {{ width:14px !important; height:14px !important; fill:{T['muted2']} !important; color:{T['muted2']} !important; }}
    [class*="st-key-seed_engine_wrap"] details[open] summary {{ margin-bottom:2px; }}
    [class*="st-key-seed_engine_wrap"] iframe {{ display:block; }}
