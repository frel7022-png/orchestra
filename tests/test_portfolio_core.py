...
def test_rank_flow_flags_direction_and_order():
    # price_hist 없이 호출 → 기준일pp는 investor_flow 자체의 최초 관측값(1/05)을 기준으로 폴백.
    # A: 오늘 보유율이 기준일보다 크게 위(UP, +5pp), B: 크게 아래(DOWN, -6pp), C: 살짝 위(+0.6pp)
    hist = _flow_df([
    ...
    key = core.FLOW_BASIS_KEY["foreign"]["누적"]  # 기준일pp
...
def test_get_flow_prev_day_ranks_uses_day_before_today():
    # 3일치. today=1/07 → prev_date=1/06까지만으로 순위 계산. price_hist 없음 → 기준일=1/05 폴백.
    hist = _flow_df([
    ...
    # 1/06 기준: B가 1/05 대비 더 크게 위(+6pp vs A +3pp) → B 1위, A 2위
...
def test_compute_foreign_flags_uses_price_history_origin_when_given():
    # price_hist를 넘기면 investor_flow 자체의 최초 관측값이 아니라 price_hist 최초 관측일
    # 값을 기준으로 삼는다 — Fishing/Link와 같은 "기준일" 개념 공유(2026-09-11).
    flow = _flow_df([
        ("X", "2026-07-24", 100, 30.0),  # 가격 추적 시작 전의 값 — 기준으로 쓰이면 안 됨
        ("X", "2026-08-19", 100, 10.0),
        ("X", "2026-08-26", 100, 12.0),
    ])
    price = _price_df([("X", "2026-08-19", 1000), ("X", "2026-08-26", 900)])
    flags_with_price = {f["종목코드"]: f for f in core.compute_foreign_flags(flow, price)}
    assert flags_with_price["X"]["기준일보유율"] == pytest.approx(10.0)
    assert flags_with_price["X"]["기준일pp"] == pytest.approx(2.0)
    # price_hist 없으면 investor_flow 자체 최초값(7/24=30.0)으로 폴백 — 다른 숫자가 나와야 함
    flags_without_price = {f["종목코드"]: f for f in core.compute_foreign_flags(flow)}
    assert flags_without_price["X"]["기준일보유율"] == pytest.approx(30.0)


def test_compute_link_candidates_reuses_compute_foreign_flags_exactly():
    # Link의 dF는 Foreigner의 기준일pp와 항상 같은 값이어야 한다(같은 함수를 재사용하므로) —
    # "데이터의 뿌리는 같아야 한다"는 사용자 설계(2026-09-11).
    price = _price_df([("X", "2026-08-19", 1000), ("X", "2026-08-26", 900)])
    flow = _flow_df([("X", "2026-08-19", 100, 10.0), ("X", "2026-08-26", 100, 13.5)])
    link_out = core.compute_link_candidates(price, flow, min_price_days=2)
    fx = {f["종목코드"]: f for f in core.compute_foreign_flags(flow, price)}
    assert link_out.iloc[0]["dF"] == pytest.approx(fx["X"]["기준일pp"])
    assert link_out.iloc[0]["기준외인비중"] == pytest.approx(fx["X"]["기준일보유율"])
    assert link_out.iloc[0]["현재외인비중"] == pytest.approx(fx["X"]["오늘보유율"])
...
def test_compute_link_candidates_demotes_reversed_recent_trend():
    # 사용자 예시(대한항공, 2026-09-11): 기준일 대비 외인비중은 +2.21%p로 "진행형"처럼 보여도,
    # 최근 3일간 오히려 빠지는 중이면 "아직 덜 먹었다"가 아니라 "슬슬 나가는 초입"일 수 있음.
    # X: 꾸준히 상승(진행중) — Y: 기준일 이후 한때 더 올랐다가(피크) 최근 3거래일 빠지는 중(꺾임).
    # 둘 다 P·|dF| 크기는 같게 맞춰서, 오직 "추세" 차이만으로 순위가 갈리는지 확인.
    price = _price_df([
        ("X", "2026-08-19", 10000), ("X", "2026-08-25", 11500),  # +15%
        ("Y", "2026-08-19", 10000), ("Y", "2026-08-25", 11500),  # +15%
    ])
    flow = _flow_df([
        ("X", "2026-08-19", 100, 10.0), ("X", "2026-08-20", 100, 10.5),
        ("X", "2026-08-21", 100, 11.0), ("X", "2026-08-22", 100, 11.5),
        ("X", "2026-08-25", 100, 12.0),  # 기준일pp=+2.0, 최근3일pp=12.0-10.5=+1.5 → 같은 부호
        ("Y", "2026-08-19", 100, 8.0), ("Y", "2026-08-20", 100, 13.0),
        ("Y", "2026-08-21", 100, 12.0), ("Y", "2026-08-22", 100, 11.0),
        ("Y", "2026-08-25", 100, 10.0),  # 기준일pp=+2.0, 최근3일pp=10.0-13.0=-3.0 → 반대 부호
    ])
    out = core.compute_link_candidates(price, flow, min_price_days=2)
    x, y = out[out["종목코드"] == "X"].iloc[0], out[out["종목코드"] == "Y"].iloc[0]
    assert x["구간"] == "진행형" and y["구간"] == "진행형"  # 둘 다 누적 기준으론 같은 구간
    assert x["dF"] == pytest.approx(2.0) and y["dF"] == pytest.approx(2.0)  # 누적 크기도 같음
    assert x["추세"] == "진행중"
    assert y["추세"] == "꺾임"
    assert y["최근3일dF"] < 0
    # 추세가 반대인 Y는 같은 누적 크기여도 X보다 뒤로 밀려야 함
    assert list(out["종목코드"]).index("X") < list(out["종목코드"]).index("Y")


def test_compute_link_candidates_classifies_four_quadrants():
    # 사용자가 지정한 4분면(2026-09-11, "설계 시 깔고 갈 전제"):
    # A: 가격↓ 외인↑("다이버전스", 곧 뭔가 일어난다) — 최우선.
    # B: 가격↑ 외인↑("진행형", 아직 덜 먹었다).
    # C: 가격↓ 외인↓("이탈", 관심에서 멀어진다 — 물타기 참고용).
    # D: 가격↑ 외인↓("차익실현", 이제 나가는 중 — 태광/성광벤드 패턴, 관심 밖).
    price = _price_df([
        ("A", "2026-08-19", 10000), ("A", "2026-08-25", 8000),    # -20%
        ("B", "2026-08-19", 10000), ("B", "2026-08-25", 11000),   # +10%
        ("C", "2026-08-19", 10000), ("C", "2026-08-25", 8000),    # -20%
        ("D", "2026-08-19", 10000), ("D", "2026-08-25", 13100),   # +31%
    ])
    flow = _flow_df([
        ("A", "2026-08-19", 100, 10.0), ("A", "2026-08-25", 100, 13.0),  # +3.0%p
        ("B", "2026-08-19", 100, 10.0), ("B", "2026-08-25", 100, 12.0),  # +2.0%p
        ("C", "2026-08-19", 100, 10.0), ("C", "2026-08-25", 100, 7.0),   # -3.0%p
        ("D", "2026-08-19", 100, 10.0), ("D", "2026-08-25", 100, 7.0),   # -3.0%p
    ])
    out = core.compute_link_candidates(price, flow, min_price_days=2)
    quad = dict(zip(out["종목코드"], out["구간"]))
    assert quad == {"A": "다이버전스", "B": "진행형", "C": "이탈", "D": "차익실현"}
    # 구간 우선순위: 다이버전스 → 진행형 → 이탈 → 차익실현 순으로 나와야 함
    assert list(out["종목코드"]) == ["A", "B", "C", "D"]
...
def test_compute_fa_win_rate_empty_transactions():
    ...
    assert core.compute_fa_win_rate(empty) == {
        "win": 0, "total": 0, "n_out": 0, "ma": 0, "mo_closed": 0,
